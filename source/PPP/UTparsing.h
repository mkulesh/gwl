...
public:
    ...
    void add (UTMultiOption & newOption)
    {
    ...
    inline void parse (int argc, char * argv[], bool checkErrors = false)
    {
        /* help menu */
        ...
        for (unsigned i = 0; i < _dataMult.size(); ++i)
        {
            ...
            if (_dataMult[i]->getOption(0)->isOptionGiven())
            {
                for (unsigned k = 0; k < n; ++k)
                {
                    ...
                    {
                        _dataMult[i]->getOption(k)->copyValueFrom(_dataMult[i]->getOption(n));
                    }
                }
            }
        }
