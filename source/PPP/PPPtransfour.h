...
template<class AType> class PPPTransFour : public PPPBaseTemplate<AType>
  ...
  public:
    ...
    typedef enum {FTTauto, FTTcft, FTTfft} FTType;
    ...
    void FT(PPPSignalContainer<PPPcomplex> &aFour,PPPSignalContainer<AType> &aVoice) {
      ...
      switch(_transType)
        {
        case FTTauto:
          if(_math.isPowerOfTwo(aVoice.points())) FFT(aFour,aVoice);
          else CFT(aFour,aVoice);
          break;
        case FTTcft:
          CFT(aFour,aVoice);
          break;
    ...
    void IFT(PPPSignalContainer<AType> &aVoice, PPPSignalContainer<PPPcomplex> &aFour, double aMin, double aMax) {
      switch(_transType)
        {
        case FTTauto:
          if(_math.isPowerOfTwo(aFour.points())) IFFT(aVoice,aFour,aMin);
          else ICFT(aVoice,aFour,aMin);
          break;
        case FTTcft:
    ...
    void FTSeparate(PPPSignalContainer<AType> &aPlus, PPPSignalContainer<AType> &aMinus, PPPSignalContainer<AType> &aFour) {
      ...
      for(unsigned c=0; c<aFour.channels(); c++)
        for(unsigned i=0; i<aFour.points()/2; i++)
          {
          aPlus(i,c) = aFour(i,c);
          if(i == 0) aMinus(i,c) = 0.0;
          else aMinus(i,c) = aFour(aFour.points()-i,c);
          }
      return;
      };

    void FTCut(PPPSignalContainer<AType> &aFour, PPPSignalContainer<AType> &aPlus, PPPSignalContainer<AType> &aMinus) {
      if(aPlus.points() != aMinus.points() || aPlus.channels() != aMinus.channels())
        PPPBaseTemplate<AType>::onError(PPPTRANSFOUR_NOTIDENT+string("FTCut"));
      unsigned flag = (2*(unsigned)(aFour.points()/2) == aFour.points())? 0:1;
      for(unsigned c=0; c<aFour.channels(); c++)
        for(unsigned i=0; i<aFour.points()/2; i++)
          {
          aFour(i,c) = aPlus(i,c);
          if(i == 0) aFour(aFour.points()/2+i,c) = 0.0;
          else aFour(aFour.points()/2+i+flag,c) = aMinus(aFour.points()/2-i,c);
          }
      return;
      };

    /** Hilbert Transforms ******************************************************/
    void HT(PPPSignalContainer<PPPcomplex> &aVoice1, PPPSignalContainer<AType> &aVoice, PPPAxis::AxisSign aT) {
      strstream str;
      str << PPPTRANSFOUR_FHT << " " << PPPBaseTemplate<AType>::getTypeName() << "[" << aVoice.channels() << "]" << ends;
      ...
      switch(aT)
        ...
        case PPPAxis::ASplus:
          h[0] = h[count/2] = 1.0;
          for(unsigned i=1; i<=count/2-1; i++) h[i]=2.0;
          break;
        case PPPAxis::ASminus:
          h[count-1] = h[count/2+1] = 1.0;
          for(unsigned i=count/2+2; i<=count-2; i++) h[i]=2.0;
          break;
        }
      ...
      PPPTransFour<PPPcomplex> trans_cmpl;  trans_cmpl.setShowMessage(false);
    ...
    /** Band Pass Filter **********************************************************/
    void BandPassFilter(PPPSignalContainer<AType> &aVoice1, PPPSignalContainer<AType> &aVoice, double aMin, double aMax) {
      strstream str;
      str << PPPTRANSFOUR_FFILT << " " << PPPBaseTemplate<AType>::getTypeName() << "[" << aMin << "..." << aMax << "]Hz" << ends;
      PPPBaseTemplate<AType>::onMessage(str.str());
      bool aSaveStatus = PPPBaseTemplate<AType>::getShowMessage();  
      PPPBaseTemplate<AType>::setShowMessage(false);
      PPPSignalContainer<PPPcomplex> aFour;
      ...
      for(unsigned c=0; c<aFour.channels(); c++)
        for(unsigned i=0; i<aFour.points(); i++)
          if(fabs(aFour.getAxis(i)) < aMin || fabs(aFour.getAxis(i)) > aMax) aFour(i,c) = 0.0;
      trans_cmpl.FTShift(aFour);
    ...
    void CCF(PPPSignalContainer<PPPcomplex> &aDest, PPPSignalContainer<AType> &aS1, PPPSignalContainer<AType> &aS2) {
      strstream str;
      str << PPPTRANSFOUR_CCF << " " << PPPBaseTemplate<AType>::getTypeName() << "[" << aS1.channels() << "]" << ends;
      PPPBaseTemplate<AType>::onMessage(str.str());
      if(aS1.points() != aS2.points()  || aS1.channels() != aS2.channels())
        PPPBaseTemplate<AType>::onError(PPPTRANSFOUR_NOTIDENT+string("CCF"));
      bool aSaveStatus = PPPBaseTemplate<AType>::getShowMessage(); 
      PPPBaseTemplate<AType>::setShowMessage(false);
    ...
    void ICCF(PPPSignalContainer<AType> &aDest, PPPSignalContainer<PPPcomplex> &aSource, double aMin, double aMax) {
      strstream str;
      str << PPPTRANSFOUR_ICCF << " " << PPPBaseTemplate<AType>::getTypeName() << "[" << aSource.channels() << "]" << ends;
      PPPBaseTemplate<AType>::onMessage(str.str());
      bool aSaveStatus = PPPBaseTemplate<AType>::getShowMessage();  PPPBaseTemplate<AType>::setShowMessage(false);
    ...
    void CCS(PPPSignalContainer<PPPcomplex> &aDest, PPPSignalContainer<AType> &aSource, PPPMatrixContainer<unsigned> &aNumb) {
      strstream str;
      str << PPPTRANSFOUR_CCS << " " << PPPBaseTemplate<AType>::getTypeName() <<" ["<<aNumb.rows()<<"]" << endl
        << "  " << aNumb.matrixToStr() << ends;
      PPPBaseTemplate<AType>::onMessage(str.str());
      bool aSaveStatus = PPPBaseTemplate<AType>::getShowMessage();  PPPBaseTemplate<AType>::setShowMessage(false);
  ...
  private:
    ...
    void CFT(PPPSignalContainer<PPPcomplex> &aFour,PPPSignalContainer<AType> &aVoice) {
      strstream str;
      str << PPPTRANSFOUR_FT << " " << PPPBaseTemplate<AType>::getTypeName() << "[" << aVoice.channels() << "]" << ends;
      PPPBaseTemplate<AType>::onMessage(str.str());
      PPPAxis ax(aVoice.points(), 0.0, aVoice.getAxis().getSamplingFreq(), PPPAxis::ATlin, PPPSPECTRCONTAINER_FREQ);
    ...
    void ICFT(PPPSignalContainer<AType> &aVoice, PPPSignalContainer<PPPcomplex> &aFour, double aMin) {
      strstream str;
      str << PPPTRANSFOUR_IFT << " " << PPPBaseTemplate<AType>::getTypeName() << "[" << aFour.channels() << "]" << ends;
      PPPBaseTemplate<AType>::onMessage(str.str());
      ...
      for(unsigned c=0; c<aFour.channels(); c++)
        for(unsigned k=0; k<aFour.points(); k++)
          {
          sprintf(currFreq,"  %s: [%d] %d",PPPTRANSFOUR_CURR,c+1,k);
          PPPBaseTemplate<AType>::onProgress(100*c*k/(aFour.points()*aFour.channels()),currFreq);
          PPPcomplex c2 = 0.0;
    ...
    void FFT(PPPSignalContainer<PPPcomplex> &aFour, PPPSignalContainer<AType> &aVoice) {
      PPPFft<AType> trans;
      strstream str;
      str << PPPTRANSFOUR_FFT << " "
          << trans.getMethodName() << ", "
          << PPPBaseTemplate<AType>::getTypeName()
    ...
    void IFFT(PPPSignalContainer<AType> &aVoice, PPPSignalContainer<PPPcomplex> &aFour, double aMin) {
      PPPFft<AType> trans;
      strstream str;
      str << PPPTRANSFOUR_IFFT << " "
          << trans.getMethodName() << ", "
          << PPPBaseTemplate<AType>::getTypeName()
          << "[" << aFour.channels() << "]" << ends;
      double aMax = aMin+(double)(aFour.points()-1)/aFour.getAxis().getMax();
