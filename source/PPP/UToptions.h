...
class UTOption_xxx : public UTOption {
  ...
  public:
    ...
    UTOption_xxx(argtabletype * opt, D const & defaultValue):
      _data(opt), _value(defaultValue), _set(false) {
      };

    UTOption_xxx(argtabletype * opt):
      _data(opt),_set(false) {
      };
    ...
    void printUsage() {
      ...
      cout << "--" << _data->hdr.longopts << " " << _data->hdr.datatype << " " <<_data->hdr.glossary << endl;
      };
	...
	virtual void
	copyValueFrom ( UTOption * option ) {
		setValue ( 
			dynamic_cast< option_type * > ( option ) -> getValue()
		);
	}
...
class UTOption_dbl : public UTOption_xxx < double, arg_dbl > {
    ...
    UTOption_dbl (
        ...
        UTOption_xxx < double, arg_dbl > (
       		arg_dbl0 (
       			shortOption, longOption, typeInfo, info
       		),
   			defaultValue
   		),
       	_unit ( 1) {
    ...
    UTOption_dbl (
        ...
        const char* info ):
     	UTOption_xxx<double, arg_dbl>(
      	arg_dbl0 (shortOption,longOption,typeInfo,info)),
       	_unit ( 1)  {
        };
    ...
    double getParsed() {
    	if (!isParsed())
          { cout << UTOPTION_ERRPARS + string("getParsed") << endl; exit(-1); }
        return _data->dval[0];
        };
  ...
  public:
    ...
    UTOption_str(
        ...
        const char* defaultValue):
            UTOption_xxx<char const*, arg_str>(
	        arg_str0(shortOption,longOption,typeInfo,info),
	        defaultValue) {
        };
...
class UTOption_int : public UTOption_xxx<int, arg_int> {
  ...
  public:
    ...
    UTOption_int(
        ...
        const char* info,
        int defaultValue):
            UTOption_xxx<int, arg_int>(
	       	arg_int0(shortOption,longOption,typeInfo,info),
	        defaultValue) {
        };
...
class UTOption_file : public UTOption_xxx<char const *, arg_file> {
  ...
  public:
    ...
    UTOption_file(
		...
		const char* info,
		const char * defaultValue):
	    UTOption_xxx<char const * , arg_file>(
	        arg_file0(shortOption,longOption,typeInfo,info),
	        defaultValue) {
        };
    ...
    UTOption_file (
        ...
        bool obligatory):
            UTOption_xxx<char const * , arg_file>(
                arg_file1(shortOption,longOption,typeInfo,info)) {
        };
...
class UTOption_lit : public UTOption_xxx<bool, arg_lit> {
  ...
  public:
	...
	const char* info,
	bool defaultValue):
	    UTOption_xxx<bool, arg_lit>(
	       	arg_lit0 (shortOption,longOption,info),
 	        defaultValue) {
        };
