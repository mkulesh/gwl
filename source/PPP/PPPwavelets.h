...
class PPPWavelet : public PPPBaseObject
  ...
  public:
    ...
    unsigned evalTimeRepresentation(PPPVectorContainer<PPPcomplex> &aDest, PPPAxis &aTime, double aCutOff, bool aReSize=true) {
      ...
      if(aReSize)
        ...
        if(aDest.size() != (2*nmax-1))
          PPPBaseObject::onError(PPPWAVELETS_ERRSIG+string("evalTimeRepresentation()"));
    ...
    double getCutoffTimeNorm(double eps) const {
      if (eps >= 1.0 || eps <= 0.0)
        onError(PPPWAVELETS_ERRCUTOFF1+string("getCutoffTimeNorm"));
    ...
    double getCutoffFreqNorm(double eps) const {
      if (eps >= 1.0 || eps <= 0.0)
        onError(PPPWAVELETS_ERRCUTOFF1+string("getCutoffFreqNorm"));
    ...
    const char *getInfo(void) {
      strstream aDest;
      aDest << getType() << ":" << endl;
      aDest << "  time-frequency position (" << getPosition() << "," << getFrequency() << ")" << endl;
      aDest << "  time maximum = " << evalMaxAmplitude() << ", time width = " << getDeltaTime() << endl;
      aDest << "  frequency maximum = " << evalMaxFourier() << ", frequency width = " << getDeltaFreq() << ends;
    ...
    friend ostream& operator << (ostream& aDest, WaveletType aSour) {
      switch(aSour)
        {
        case CWnone:      aDest << PPPWAVELETS_NONE; break;
        case CWmorlet:    aDest << PPPWAVELETS_MORLET; break;
        case CWmorletre:  aDest << PPPWAVELETS_MORLETRE; break;
        case CWcauchy:    aDest << PPPWAVELETS_CAUCHY; break;
        case CWshanon:    aDest << PPPWAVELETS_SHANON; break;
        case CWhaar:      aDest << PPPWAVELETS_HAAR; break;
        case CWdelta:     aDest << PPPWAVELETS_DELTA; break;
        }
      return aDest;
      };
  ...
  private:
    ...
    PPPcomplex Chm_IntegralTrapez(double a, double b, unsigned aSize, PPPWavelet *inv) {
      ...
      if (aSize == 1)
        ...
        del=(b-a)/tnm;
        x=a+0.5*del;
        for (sum=0.0,j=1;j<=it;j++,x+=del) sum += Chm_func(x,inv);
        s=((b-a)*sum/tnm);
        return s;
        }
      };
