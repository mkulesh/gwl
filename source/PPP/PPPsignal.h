...
public:
    ...
    void prepare (const unsigned aPoints, const unsigned aChannels, PPPAxis &aAxis,
                  const string &aN1)
    {
    ...
    {
        strstream aDest;
        aDest << PPPBaseTemplate<AType>::getObjectName() << ": "
        << PPPBaseTemplate<AType>::getTypeName() << " " << PPPSIGNALCONTAINER_SEC << endl;
     ...
     */
    void read (const string &aName, bool aIsTime = true, double aSampleFreq = 0.0,
               double aMin = 0.0, double aMax = 0.0)
    {
        ...
        if (ind2 < ind1 || ind1 > Snew.rows() || ind2 > Snew.rows()) PPPBaseObject::onError(
                ARG_VALUE + string("Read"));
    ...
    void write (const string &aName, TransType aTrans, const bool aIsTime = false,
                const bool aRepName = false)
    {
        string newName(aName);
        if (aRepName) newName.replace(newName.find("."), 1,
                                      string(aTrans.getComponentCode()) + ".");
        strstream str;
        str << FILE_WRITEASC << " " << newName << " [" << points() << "][" << channels() << "], "
        << aTrans.getComponentName() << ends;
    ...
    void toPowerOfTwo (const unsigned aBase = 0)
    {
        ...
        for (unsigned i = 0; i < channels(); i++)
            _data[i].resize(_axis.size());
    }
    
    void rotation (const unsigned aChan1, const unsigned aChan2, const double aPhi)
    {
        if (aChan1 >= channels() || aChan2 >= channels()) PPPBaseObject::onError(
                MEM_ERRINDEX + string("Rotation"));
        AType v1, v2;
        for (unsigned i = 0; i < points(); i++)
        {
    ...
    void integration (void)
    {
        ...
        for (unsigned chann = 0; chann < channels(); chann++)
        {
            ...
            {
                n3 = n2 + (n1 + 4.0 * (*this)(i, chann) + (*this)(i + 1, chann))
                        / (6.0 * getAxis().getSamplingFreq());
                n1 = (*this)(i, chann);
                n2 = n3;
                (*this)(i, chann) = n3;
            }
    ...
    void differentiation (PPPSignalContainer<AType> &aDest)
    {
        ...
        for (unsigned chann = 0; chann < channels(); chann++)
        {
            aDest(0, chann) = 0.0;
            for (unsigned i = 1; i < points() - 1; i++)
                aDest(i, chann) = ((*this)(i, chann) - (*this)(i - 1, chann))
                        / (getAxis(i) - getAxis(i - 1));
        }
    }
        ...
        for (unsigned chann = 0; chann < channels(); chann++)
        {
            ...
            for (unsigned i = 1; i < points() - 1; i++)
            {
                ...
                if (abs(f2) == 0.0)
                    ...
                    yd[i] = (f2.real() * (f3.imag() - f1.imag())
                            - f2.imag() * (f3.real() - f1.real()))
                            / (dt * abs(f2) * abs(f2));
            }
            yd[0] = yd[1];
    ...
    AType Get (double aX, const unsigned aChannel = 0, InterpType aC = ITlin)
    {
        ...
        case ITsplin:
            ...
            break;
        default:
            PPPBaseObject::onError(PPPSIGNALCONTAINER_ERRINTERP + string("Get"));
        }
        return s;
    }
    ...
    void prepareSpline (const unsigned aChannel = 0)
    {
        ...
        un = (3.0 / (x[count - 1] - x[count - 2])) * (aYpn
                - (y[count - 1] - y[count - 2]) / (x[count - 1] - x[count - 2]));
        _splineTmp[count - 1] = (un - qn * u[count - 2]) / (qn * _splineTmp[count - 2] + 1.0);
        for (k = count - 1; k >= 1; k--)
            _splineTmp[k - 1] = _splineTmp[k - 1] * _splineTmp[k] + u[k - 1];
        return;
    }
    ...
    AType evalSplineInterpolation (double aX, const unsigned aChannel = 0)
    {
        ...
        return (a * y[klo - 1] + b * y[khi - 1]
                + ((a * a * a - a) * _splineTmp[klo - 1] + (b * b * b - b) * _splineTmp[khi - 1]) * (h
                        * h)
                  / 6.0);
    }
