...
class PPPVectorContainer : public PPPBaseTemplate<AType> {
  ...
  public:
    ...
    void realloc(const unsigned aSize) { // reallocate to size aSize
    ...
    void resize(const unsigned aSize) {
      ...
      if(aSize == 0 && size() != 0)
        ...
        for(unsigned i=0; i<aSize; i++) _data[i] = PPPBaseTemplate<AType>::nullValue();
        _size = aSize;
        return;
        }
      else if(aSize != 0 && size() != 0)
        {
        AType *_data1;
        if((_data1 = new AType[aSize]) == NULL) PPPBaseTemplate<AType>::onError(MEM_ERRALLOC+string("resize"));
        for(unsigned i=0; i<aSize; i++)
          {
          if(i<size()) _data1[i] = _data[i];
          else _data1[i] = PPPBaseTemplate<AType>::nullValue();
          }
        delete _data;
        _data = _data1;
        _size = aSize;
    ...
    void link(PPPVectorContainer & aSour, 		 // the source to which we link
      ...
      _link(aSour._data+aSour._offset*index_beg, &(aSour._childscount), size, aOffset*aSour._offset);
      return;
      };
    ...
    void assign(const AType aVal) {
      for(unsigned i=0; i<size(); i++) (*this)[i] = aVal;
      return;
      };
    ...
    void assign(const PPPVectorContainer & aSour, unsigned aStart=0, unsigned aSize=0, int aOffset=1) {
      ...
      if(newsize == 0 && aOffset != 1)
        newsize = (unsigned)ceil((double)(aSour.size()-aStart)/((double)aOffset));
      realloc(newsize);
    ...
    void compTransform(PPPVectorContainer<DestType> &aDest, TransType aTrans) {
      ...
      for(unsigned i=0; i<size(); i++) aDest[i] = aTrans((*this)[i]);
      return;
      }

    const char *getInfo(void) {
      strstream str;
      str << PPPBaseTemplate<AType>::getObjectName() << ": " << PPPBaseTemplate<AType>::getTypeName() << "[" << size() << "]";
    ...
    void write(const string &aName, TransType &aTrans) {
      strstream str;
      str << FILE_WRITEASC << aName <<"["<<size()<<"], "<<aTrans.getComponentName()<<ends;
    ...
    void strToVector(const string &aSour) {
      ...
      str[vallen] = 0;
      for(size=0, uk1=str; *uk1 != 0; uk1++) if(*uk1 == ',') size++;
      size++;
      resize(size);
      uk1 = uk2 = &str[0];
      size = 0;
    ...
    AType getSumm (void) {
      ...
      for(unsigned i=0; i<size(); i++) val = val + (*this)[i];
      return val;
      };
    ...
    AType getMinValue (TransType &aTrans) {
      ...
      for(unsigned i=0; i<size(); i++)
        {
        nres = aTrans((*this)[i]);
        if(i==0 || nres < res) {  res=nres; val=(*this)[i]; }
        }
      return val;
      }
    ...
    void toPowerOfTwo(unsigned aBase=0) {
      ...
      if(aBase == 0)
        ...
        {
        if(!math.isPowerOfTwo(aBase))
          PPPBaseTemplate<AType>::onError(ARG_POW2+string("ToPowerOfTwo"));
        }
      resize(aBase);
      return;
      }
