...
typedef enum
{
    WTERene, WTEMorozov, WTESumCovar, WTECovar, WTEComplex, WTEMlinet, WTEMlinef
} PPPConstETmachine;
...
ostream& operator << (ostream& aDest, PPPConstETmachine aSour)
{
    ...
    {
    case WTERene:
        aDest << PPPELLIPSE_RENE;
        break;
    case WTEMorozov:
        aDest << PPPELLIPSE_MOROZOV;
        break;
    case WTESumCovar:
        aDest << PPPELLIPSE_SUMCOVAR;
        break;
    case WTECovar:
        aDest << PPPELLIPSE_COVAR;
        break;
    case WTEComplex:
        aDest << PPPELLIPSE_COMPLEX;
        break;
    case WTEMlinet:
        aDest << PPPELLIPSE_MLINET;
        break;
    case WTEMlinef:
        aDest << PPPELLIPSE_MLINEF;
        break;
    }
    return aDest;
}
...
public:
    ...
    void set (double const b)
    {
        for (unsigned i = 0; i < N; i++)
            rmin[i] = rmax[i] = rmidd[i] = inst[i] = b;
        tilt = phi = phase1 = phase2 = b;
    }
    
    // eigenvectors
    double vecmax (unsigned aInd) const
    {
        return rmax[aInd];
    }
    
    double vecmin (unsigned aInd) const
    {
        return rmin[aInd];
    }
    
    double vecmidd (unsigned aInd) const
    {
        return rmidd[aInd];
    }
    ...
    {
        aC1 = PPPcomplex(rmax[0] * cos(phi) - rmin[0] * sin(phi),
                         rmax[0] * sin(phi) + rmin[0] * cos(phi));
        aC2 = PPPcomplex(rmax[1] * cos(phi) - rmin[1] * sin(phi),
                         rmax[1] * sin(phi) + rmin[1] * cos(phi));
        aC3 = PPPcomplex(rmax[2] * cos(phi) - rmin[2] * sin(phi),
                         rmax[2] * sin(phi) + rmin[2] * cos(phi));
        return;
    }
    ...
    void CovarianceSumParams (PPPVectorContainer<double> &aV1, PPPVectorContainer<double> &aV2)
    {
        ...
        for (k = 0; k < 2; k++)
            for (l = 0; l < 2; l++)
                cov(k, l) = 2.0 * cov(k, l) / ((double) aV1.size());
    ...
    double CovarianceInteg (PPPcomplex aZx, PPPcomplex aZy, double aWx, double aWy, double aT)
    {
        ...
        double val =
                abs(aZx) * abs(aZy)
                * (mfunc.sinc(aT * (aWy - aWx) / 2.0) * cos(arg(aZy) - arg(aZx)) + mfunc.sinc(
                        aT * (aWy + aWx) / 2.0)
                                                                                   * cos(arg(aZy) + arg(
                                                                                           aZx)));
        double mean = (mfunc.sinc(aT * aWx / 2.0) * aZx.real())
                * (mfunc.sinc(aT * aWy / 2.0) * aZy.real());
        return val - mean;
    }
    
    void CovarianceParams (PPPcomplex &aC1, PPPcomplex &aC2, double aW1, double aW2,
                           unsigned aN = 1)
    {
        set(0.0);
        // Calculation of covariance matrix
    ...
    void CovarianceParams (PPPcomplex &aC1, PPPcomplex &aC2, PPPcomplex &aC3, double aW1,
                           double aW2, double aW3, unsigned aN = 1)
    {
        set(0.0);
        // Calculation of covariance matrix
        PPPMatrixContainer<double> cov(3, 3);
