...
public:
    ...
    void WT (PPPSpectrContainer<PPPcomplex> &aCWT, PPPSignalContainer<AType> &aVoice,
             PPPSpectrParams &aT)
    {
        // wavelet
        ...
        case ATtimeInt:
            ...
            << wav->getInfo() << endl << "  " << PPPTRANSWAVELET_WAVCUTOFF << " = "
            << aT.getCutoffPrec();
            if (wav->hasParams()) str << endl << "  " << PPPTRANSWAVELET_WAVPAR << " = "
                                  << aT.getWaveletPar(PPPSpectrParams::WSTdirect);
            str << ends;
            break;
        case ATfft:
        ...
        // wavelet container
        aCWT.prepare(aT.voices(), aVoice.points(), aVoice.channels(), aVoice.getAxis(),
                     aT.getFreq(), PPPSPECTRCONTAINER_NAME);
        // trace
        bool aTrace = (_tracename != "NULL");
        PPPSignalContainer<double> _trace;
        if (aTrace) _trace.prepare(aCWT.voices(), 1, aCWT.getFreq(), PPPTRANSWAVELET_SWTTRACE);
        ...
        else
        {
            ...
            _convolutor = new PPPConvolutor<PPPcomplex>(nsig0, // (in) lower index of first signal (out) actually needed lower index
        ...
        for (unsigned i = 0; i < aCWT.voices(); i++)
        {
            ...
            cThen4 += cThen3;
            if (aTrace) _trace(i) = cThen3;
        }
        // clock information
        strstream strcl;
        strcl << "preparation time: " << (cThen2 - cThen1) << ", calculation time: " << cThen4
        << " ticks" << ends;
        ...
        if (aTrace) _trace.write(_tracename);
        PPPBaseTemplate<AType>::onProgress(-1, "");
    }
    
    /** Inverse Wavelet Transformation ******************************************/
    void IWT (PPPSignalContainer<AType> &aVoice, PPPSpectrContainer<PPPcomplex> &aCWT,
              PPPSpectrParams &aT, bool aAmpl = true)
    {
        ...
        str << PPPTRANSWAVELET_IWT << " " << PPPBaseTemplate<AType>::getTypeName() << "["
        << aCWT.channels() << "]" << endl << aT.getWavelet(PPPSpectrParams::WSTinverse)->getInfo()
        << endl << "  " << PPPTRANSWAVELET_WAVPAR << " = "
        << aT.getWaveletPar(PPPSpectrParams::WSTdirect) << endl << "  " << PPPTRANSWAVELET_AMPLF
        << " = " << Chm << endl << "  " << PPPTRANSWAVELET_WAVCUTOFF << " = " << aT.getCutoffPrec()
        << ends;
        PPPBaseObject::onMessage(str.str());
    ...
    void IFWT (PPPSignalContainer<AType> &aVoice, PPPSpectrContainer<PPPcomplex> &aCWT,
               PPPSpectrParams &aT, bool aAmpl = true)
    {
        strstream str;
        PPPcomplex Chm(1.0, 0.0);
        if (aAmpl) Chm = aT.getInverseConstant();
        str << PPPTRANSWAVELET_IFWT << " " << PPPBaseTemplate<AType>::getTypeName() << "["
        << aCWT.channels() << "]" << endl << "  " << PPPTRANSWAVELET_AMPLF << " = " << Chm << ends;
        PPPBaseObject::onMessage(str.str());
    ...
    void WaveletCat (PPPSpectrContainer<AType> &aDest, PPPSpectrContainer<AType> &aPlus,
                     PPPSpectrContainer<AType> &aMinus)
    {
        if ((aMinus.voices() != aPlus.voices()) || (aMinus.points() != aPlus.points())
            || (aMinus.channels() != aPlus.channels())) PPPBaseObject::onError(
                ARG_VALUE + string("WaveletCat"));
        PPPAxis ax;
        ax.assign(aPlus.getFreq());
        ax.fillsign(PPPAxis::ASfull);
        aDest.prepare(2 * aPlus.voices(), aPlus.points(), aPlus.channels(), aPlus.getTime(), ax,
        PPPTRANSWAVELET_FULL);
        unsigned i, j;
    ...
    {
        if (aSource.getFreq(0) * aSource.getFreq(aSource.voices() - 1) >= 0) PPPBaseObject::onError(
                ARG_VALUE + string("WaveletSeparate"));
        unsigned i, j, k;
                ...
                TimeFreqType aType, PPPAxis::AxisSign aT)
    {
        ...
        {
        case TFTtime:
            str << PPPTRANSWAVELET_RIDCALC << ": " << PPPSIGNALCONTAINER_AXISNAME << "-"
            << PPPSPECTRCONTAINER_FREQ << ends;
            PPPBaseObject::onMessage(str.str());
            ...
            break;
        case TFTfreq:
            str << PPPTRANSWAVELET_RIDCALC << ": " << PPPSPECTRCONTAINER_FREQ << "-"
            << PPPSIGNALCONTAINER_AXISNAME << ends;
            PPPBaseObject::onMessage(str.str());
            ...
            for (j = 0; j < aCWT.voices(); j++)
            {
                ...
                aRidge.getAxis()[j] =
                        (aT == PPPAxis::ASplus) ? aCWT.getFreq(j) :
                                                  aCWT.getFreq(aCWT.voices() - 1 - j);
              ...
              PPPSpectrContainer<PPPcomplex> &aCWT, unsigned aCat = 0)
    {
        ...
        {
            double p = 0.0;
            for (i = aCat; i < (aCWT.points() - aCat); i++)
                p += pow(abs(aCWT(j, i)), 2.0);
...
private:
                               ...
                               double aCutOff = 0.0)
    {
        ...
        for (unsigned c = 0; c < aCWT.channels(); c++)
        {
            for (unsigned j = 0; j < aCWT.points(); j++)
                aCWT(aInd, j, c) = _evalIntegralInTime(aVoice.getChannel(c), j, w,
                                                       aWaxis.getSamplingPeriod());
        }
        clock_t cThen2 = clock();
    ...
    clock_t _evalVoiceConvAnalyt (PPPSpectrContainer<PPPcomplex> &aCWT, unsigned const aInd,
                                  PPPSignalContainer<PPPcomplex> &aFour, PPPAxis &aWaxis,
                                  PPPWavelet &aWav)
    {
        clock_t cThen1 = clock();
        PPPVectorContainer<PPPcomplex> w;
