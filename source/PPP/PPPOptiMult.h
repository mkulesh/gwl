...
public:
    ...
    void prepare (PPPSignalContainer<double> &aSource, PPPDispersionModel &aModel, double aDist,
                  PPPPropagatorDiss::PropType aPropType)
    {
        _sn.assign(aSource);
        if (_chan != 0) delete _opt;
        _chan = aSource.channels() - 1;
        if (_chan == 0) PPPBaseObject::onError(PPPOPTIMULT_ERRCHANN + string("prepare"));
        ...
        if (!ATypeOpti::isInitialized()) ATypeOpti::resize(_chan * _points, aModel.params());
        ATypeOpti::evalFixed((getModel().getWn().getType() == PPPApproximate::APTpolin), false,
                             getModel().getWn().size());
    }
    
    void prepare (PPPSpectrContainer<PPPcomplex> &aSource, PPPDispersionModel &aModel, double aDist,
                  PPPPropagatorDiss::PropType aPropType, PPPSpectrParams *aWtpar,
                  unsigned aCmplType)
    {
        _wgn.assign(aSource);
        if (_chan != 0) delete _opt;
        ...
        for (unsigned i = 0; i < _chan; i++)
        {
            _opt[i].prepare(aSource, aModel, (double) (i + 1) * aDist, aPropType, aWtpar, aCmplType,
                            0, i + 1);
            if (i == 0) _points = _opt[i].points();
        }
        if (!ATypeOpti::isInitialized()) ATypeOpti::resize(_chan * _points, aModel.params());
        ATypeOpti::evalFixed((getModel().getWn().getType() == PPPApproximate::APTpolin),
                             (aCmplType == TCarg), getModel().getWn().size());
    }
    
    double getcostfunc (unsigned Ai)
    {
        _indi = Ai / _points;
        if (_indi >= _chan) PPPBaseObject::onError(PPPOPTIMULT_ERROUT + string("getcostfunc"));
    ...
    {
        _indj = Aj / _points;
        if (_indj >= _chan) PPPBaseObject::onError(PPPOPTIMULT_ERROUT + string("getder"));
    ...
    {
        if (_chan == 0) PPPBaseObject::onError(PPPOPTIMULT_ERRCHANN + string("getModel"));
