...
public:
    ...
    {
        onError(VIRT_NOTDEF + string("setParams"));
        return;
    }
    ...
    void Minimize (void)
    {
        ...
        {
            sprintf(ss, "%s: %d, %s: %d, %s: %8.7e", PPPOPTILM_ITER, k, PPPOPTILM_SUBITER, itst,
            PPPOPTILM_COST,
                    _chisq);
            if (getShowProgress()) onMessage(ss);
            oochisq = _chisq;
            LevenbergMarq_min();
            if (_chisq <= oochisq && fabs(oochisq - _chisq) < MaxEps)
                itst++;
            else
                itst = 0;
        ...
        if ((params() - fixed()) > 0) str << "  " << PPPOPTILM_FAXPAR1 << " = "
                                      << fixpar.vectorToStr() << endl;
...
private:
    ...
    void LevenbergMarq_covsrt (PPPMatrixContainer<double> &aCovar)
    {
        ...
        for (j = params(); j >= 1; j--)
        {
            ...
            {
                for (i = 1; i <= params(); i++)
                    SWAP(aCovar(i - 1, k - 1), aCovar(i - 1, j - 1));
                for (i = 1; i <= params(); i++)
                    SWAP(aCovar(k - 1, i - 1), aCovar(j - 1, i - 1));
                k--;
            }
        }
        return;
    ...
    double LevenbergMarq_cof (PPPVectorContainer<double> &aPar, PPPMatrixContainer<double> &aAlpha,
                              PPPVectorContainer<double> &aBeta)
    {
        unsigned j, k, l, m;
        double sig2i, dy;
        for (j = 1; j <= fixed(); j++)
            ...
            for (j = 0, l = 1; l <= params(); l++)
            {
                if (fixpar[l - 1])
                {
                    ...
                    for (k = 0, m = 1; m <= l; m++)
                        ...
                        {
                            k++;
                            der1 = aAlpha(j - 1, k - 1) + getder(l - 1, i - 1)
                                    * getder(m - 1, i - 1) * sig2i;
                            aAlpha(j - 1, k - 1) = der1;
                        }
    ...
    void LevenbergMarq_min (void)
    {
        ...
        for (j = 0, l = 1; l <= params(); l++)
            if (fixpar[l - 1]) atry[l - 1] = optpar[l - 1] + minda[++j - 1];
