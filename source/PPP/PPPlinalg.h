...
public:
    ...
    void matrixInversion (PPPMatrixContainer<AType> &aMat, PPPVectorContainer<AType> &aB,
                          unsigned aSize)
    {
        if (aMat.cols() != aMat.rows()) PPPBaseTemplate<AType>::onError(
                PPPLINALG_ERRQUAD + string("MatrixInversion"));
        ...
        for (i = 1; i <= aSize; i++)
        {
            big = 0.0;
            for (j = 1; j <= aSize; j++)
                if (ipiv[j - 1] != 1) for (k = 1; k <= aSize; k++)
                {
                    if (ipiv[k - 1] == 0)
                    {
            ...
            {
                for (l = 1; l <= aSize; l++)
                    SWAP(aMat(irow - 1, l - 1), aMat(icol - 1, l - 1));
                SWAP(aB[irow - 1], aB[icol - 1]);
            }
            indxr[i - 1] = irow;
            indxc[i - 1] = icol;
            if (aMat(icol - 1, icol - 1) == 0.0) PPPBaseTemplate<AType>::onError(
                    PPPLINALG_ERRSING + string("MatrixInversion"));
            pivinv = 1.0 / aMat(icol - 1, icol - 1);
            aMat(icol - 1, icol - 1) = 1.0;
            for (l = 1; l <= aSize; l++)
                aMat(icol - 1, l - 1) *= pivinv;
            aB[icol - 1] *= pivinv;
            for (ll = 1; ll <= aSize; ll++)
                if (ll != icol)
                {
                    dum = aMat(ll - 1, icol - 1);
                    aMat(ll - 1, icol - 1) = 0.0;
                    for (l = 1; l <= aSize; l++)
                        aMat(ll - 1, l - 1) -= aMat(icol - 1, l - 1) * dum;
    ...
    void multiplication (PPPMatrixContainer<AType> &aDest, PPPMatrixContainer<AType> &aM1,
                         PPPMatrixContainer<AType> &aM2)
    {
        if (aM1.rows() != aM2.cols() || aM1.cols() != aM2.rows()) PPPBaseTemplate<AType>::onError(
                PPPLINALG_ERRSIZE + string("Multiplication"));
        ...
        for (k = 0; k < aM1.cols(); k++)
            ...
            {
                sum = 0.0;
                for (i = 0; i < aM1.cols(); i++)
                    sum = sum + aM1(j, i) * aM2(i, k);
                aDest(j, k) = sum;
            }
    }
    
    // multiplication of matrix and vector
    void multiplication (PPPVectorContainer<AType> &aDest, PPPMatrixContainer<AType> &aM1,
                         PPPVectorContainer<AType> &aM2)
    {
        if (aM1.cols() != aM2.size()) PPPBaseTemplate<AType>::onError(
                PPPLINALG_ERRSIZE + string("Multiplication"));
    ...
    void jacobi (PPPMatrixContainer<double> &aMat, PPPVectorContainer<double> &aEVal,
                 PPPMatrixContainer<double> &aEVec)
    {
        if (!isSquare(aMat)) PPPBaseTemplate<AType>::onError(PPPLINALG_ERRQUAD + string("Jacobi"));
        if (!isSymetric(aMat)) PPPBaseTemplate<AType>::onError(
                PPPLINALG_ERRSYMM + string("Jacobi"));
        unsigned j, iq, ip, i;
        ...
        for (i = 1; i <= 50; i++)
        {
            ...
            for (ip = 1; ip <= n - 1; ip++)
            {
                for (iq = ip + 1; iq <= n; iq++)
                {
                    ...
                    if (i > 4 && (double) (fabs(aEVal[ip - 1]) + g) == (double) fabs(aEVal[ip - 1])
                        && (double) (fabs(aEVal[iq - 1]) + g) == (double) fabs(aEVal[iq - 1]))
                        aMat(ip - 1, iq - 1) = 0.0;
                    else if (fabs(aMat(ip - 1, iq - 1)) > tresh)
                    {
                        ...
                        aMat(ip - 1, iq - 1) = 0.0;
                        for (j = 1; j <= ip - 1; j++)
                            _jacobiRot(aMat, j, ip, j, iq, s, tau);
                        for (j = ip + 1; j <= iq - 1; j++)
                            _jacobiRot(aMat, ip, j, j, iq, s, tau);
                        for (j = iq + 1; j <= n; j++)
                            _jacobiRot(aMat, ip, j, iq, j, s, tau);
                        for (j = 1; j <= n; j++)
                            _jacobiRot(aEVec, j, ip, j, iq, s, tau);
                        nrot++;
                    }
                }
            }
    ...
    void eigComplex (PPPMatrixContainer<PPPcomplex>& aMat, PPPVectorContainer<double>& aEVal,
                     PPPMatrixContainer<PPPcomplex>& aEVec)
    {
        if (!isHermetian(aMat)) PPPBaseTemplate<AType>::onError(
                PPPLINALG_ERRHERM + string("EigComplex"));
    ...
    void mean (unsigned l, PPPVectorContainer<AType>& v, PPPMatrixContainer<AType>& m)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            v.resize(m.rows()); // TODO: please check there was rows() only
            for (i = 0; i < m.rows(); i++)
                for (j = 0; j < m.cols(); j++)
                    v[i] = v[i] + m(j, i) / m.cols();
            break;
        default:
            PPPBaseTemplate<AType>::onError(PPPLINALG_ERRPAR + string("mean"));
        }
    }
    
    unsigned upDiagonal (PPPMatrixContainer<AType>& a)
    {
        if (!isSquare(a)) PPPBaseTemplate<AType>::onError(PPPLINALG_ERRQUAD + string("upDiagonal"));
    ...
    {
        if (!isSquare(aMat)) PPPBaseTemplate<AType>::onError(
                PPPLINALG_ERRQUAD + string("evalDeterminant"));
    ...
    AType evalDeterminantCmpl (PPPMatrixContainer<AType> &aMat)
    {
        ...
        for (unsigned i = 0; i < aMat.rows(); i++)
            d *= aMat(i, i);
        return d * p;
    }
...
private:
    ...
    void _jacobiRot (PPPMatrixContainer<double> &a, unsigned i, unsigned j, unsigned k, unsigned l,
                     double s, double tau)
    {
        double g, h;
        g = a(i - 1, j - 1);
