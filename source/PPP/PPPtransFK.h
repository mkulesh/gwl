...
class PPPTransFK : public PPPBaseObject
  ...
  public:
    ...
    typedef enum {MMIindex, MMIspline} InterpType;
    typedef enum {MMCabs, MMCarg, MMCcphase} CorrType;
  ...
  public:
    ...
    PPPTransFK(void) : _inttype(MMIindex), _corrtype(MMCarg), _filter(0.0), _norm(true), _distance(0.0) {
    ...
    friend ostream& operator << (ostream& aDest, InterpType aSour) {
      switch(aSour)
        {
        case MMIindex:  aDest << "indexed shift"; break;
        case MMIspline: aDest << "spline interpolation"; break;
        }
      return aDest;
      };
    ...
    friend ostream& operator << (ostream& aDest, CorrType aSour) {
      switch(aSour)
        {
        case MMCabs:  aDest << "correlation of modulus"; break;
        case MMCarg:  aDest << "correlation of phase"; break;
        case MMCcphase: aDest << "correlation of complex phase"; break;
        }
      return aDest;
      };

    void setIntType(InterpType const aType) { _inttype=aType; };
    void setCorrType(CorrType const aType) { _corrtype=aType; };
    void setFilter(double const aFilter) { _filter=aFilter; };
    void setNormVoice(bool const aType) { _norm=aType; };
    void setDistance(double adX) { _distance=adX; };

    void evalSpectrCorrelation(PPPSpectrContainer<double> &aFK, PPPSpectrContainer<PPPcomplex> &aWg, PPPAxis &aVel, int aTestVoice=-1) {
      ...
      strstream aDest;
      aDest << "calculation of spectrum correlation:" << endl
           << "  " << aFK.getTime().getInfo() << endl
           << "  " << aFK.getFreq().getInfo() << endl
           << "  type of interpolation: " << _inttype << endl
           << "  type of correlation: " << _corrtype << endl
           << "  value of energy filter = " << _filter << endl;
      if(_norm) aDest << "  normalization of voices: on" << ends;
                else aDest << "  normalization of voices: off" << ends;
      onMessage(aDest.str());
      char currFreq[100];
      double shift, amax;
      ...
      for(unsigned i=in_start; i<in_end; i++)
        ...
        amax = 0.0;
        if(_inttype == MMIindex) for(unsigned n=0; n<aFK.points(); n++)
          {
        ...
        else if(_inttype == MMIspline) for(unsigned n=0; n<aFK.points(); n++)
          {
        ...
        else onError(PPPTRANSFK_ERRINT+string("evalSpectrCorrelation()"));
    ...
    void shiftVoices(PPPSignalContainer<PPPcomplex> &aDest, PPPSignalContainer<PPPcomplex> &aSource, InterpType aType, double aInd) {
      ...
      if(aType == MMIindex)
        ...
        for(k=0; k<aSource.channels(); k++)
          for(j=0; j<aSource.points(); j++)
            ...
            if(ni < aSource.points()) aDest(j,k) = aSource(ni,k);
            else aDest(j,k) = 0.0;
            }
        }
        ...
        for(k=0; k<aSource.channels(); k++)
          ...
          for(j=0; j<aSource.points(); j++)
            {
            double nx = aSource.getAxis().getSamplingPeriod()*((double)j+aInd*(double)k);
            aDest(j,k) = aSource.evalSplineInterpolation(nx,k);
            }
          }
        }
      else onError(PPPTRANSFK_ERRINT+string("shiftVoices()"));
      };

    double evalVoicesCorrelation(PPPSignalContainer<PPPcomplex> &aSource, unsigned aInd,PPPVectorContainer<double> &aMod) {
        ...
        case MMCabs: case MMCarg:
          for(j=0; j<aSource.points(); j++)
            ...
            for(k=0; k<aSource.channels(); k++)
              {
              ni = j+aInd*k;
              if(ni < aSource.points()) { cmpl=aSource(ni,k); } else { prod=0.0; break; }
              if(_corrtype == MMCabs) val = abs(aSource(ni,k))/aMod[k];
                else
              if(_corrtype == MMCarg && 100.0*abs(aSource(ni,k))/aMod[k] >= _filter) val = arg(aSource(ni,k));
                else
              { prod=0.0; break; }
              prod *= val;
              }
