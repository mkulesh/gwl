...
public:
    ...
    inline unsigned points (void)
    {
        return _points;
    }
    
    inline void setPoints (unsigned apoints)
    {
        _points = apoints;
    }
    ...
    {
        if (_par.size() != 2) PPPBaseObject::onError(PPPSIGNALGEN_ERRPAR + string("genDelta()"));
        PPPAxis x = aDest.getAxis();
        unsigned pos = x.locateFloor(_par[0]);
        for (unsigned i = 0; i < aDest.points(); i++)
            aDest(i, aNum) = (AType) 0.0;
    ...
    {
        if (_par.size() != 7) PPPBaseObject::onError(PPPSIGNALGEN_ERRPAR + string("genHarmon()"));
        PPPAxis x = aDest.getAxis();
        for (unsigned i = 0; i < aDest.points(); i++)
        {
        ...
        // Notation
        strstream str;
        str << PPPSIGNALGEN_HARMON << ": " << endl << "  Sre(t) = (" << _par[0] << showpos
        << _par[1] << noshowpos << "e(t))*cos(2*Pi*" << _par[2] << "*t)" << endl;
        if (PPPBaseTemplate<AType>::isComplex()) str << "  Sim(t) = (" << _par[3] << showpos
                                                 << _par[4] << noshowpos << "e(t))*sin(2*Pi*"
                                                 << _par[5] << "*t + " << _par[6] << ")" << endl;
        str << "  e(t) = 0...1" << ends;
        PPPBaseObject::onNotation(str.str());
        return;
    }
    ...
    {
        if (_par.size() != 4) PPPBaseObject::onError(
                PPPSIGNALGEN_ERRPAR + string("genHarmonPhase()"));
        PPPAxis x = aDest.getAxis();
        for (unsigned i = 0; i < aDest.points(); i++)
        {
            PPPcomplex f = PPPcomplex(
        ...
        // Notation
        strstream str;
        str << PPPSIGNALGEN_HARMONPHASE << ": " << endl << "  Sre(t) = " << _par[0] << "*sin(2*Pi*"
        << _par[2] << "*t+sin(" << _par[3] << "*t))" << endl;
        if (PPPBaseTemplate<AType>::isComplex()) str << "  Sim(t) = " << _par[1] << "*sin(2*Pi*"
                                                 << _par[2] << "*t)" << endl;
        str << ends;
        PPPBaseObject::onNotation(str.str());
        return;
    ...
    {
        if (_par.size() != 7) PPPBaseObject::onError(
                PPPSIGNALGEN_ERRPAR + string("genHarmonRot()"));
        PPPAxis x = aDest.getAxis();
        for (unsigned i = 0; i < aDest.points(); i++)
        {
            double e = (double) i / (double) (aDest.points() - 1);
        ...
        // Notation
        strstream str;
        str << PPPSIGNALGEN_HARMONROT << ": " << endl << "  S1(t) = (" << _par[0] << showpos
        << _par[1] << noshowpos << "e(t))*cos(2*Pi*" << _par[4] << "*t)" << endl << "  S2(t) = ("
        << _par[2] << showpos << _par[3] << noshowpos << "e(t))*sin(2*Pi*" << _par[5] << "*t)"
        << endl << "  Sre(t) = S1(t)*cos(2*Pi*" << _par[6] << "*x)+S2(t)*sin(2*Pi*" << _par[6]
        << "*t)" << endl;
        if (PPPBaseTemplate<AType>::isComplex()) str << "  Sim(t) =-S1(t)*sin(2*Pi*" << _par[6]
                                                 << "*x)+S2(t)*cos(2*Pi*" << _par[6] << "*t)"
                                                 << endl;
        str << "  e(t) = 0...1" << ends;
        PPPBaseObject::onNotation(str.str());
        return;
    ...
    {
        if (_par.size() != 6) PPPBaseObject::onError(
                PPPSIGNALGEN_ERRPAR + string("genRickerDissip()"));
        // Frequency axis
        ...
        for (unsigned i = 0; i < aFour.size(); i++)
        {
            ...
            aFour[i] = (2.0 * omega * omega / (Vr * Vr * sqrt(M_PI)))
                    * exp(-(omega - om0) * (omega - om0) / (Vr * Vr));
        }
        ...
        str << "  YR1(om) = 2*om^2*exp(-om^2/Vr^2)/(Vr^2*sqrt(Pi)), Vr=" << Vr << ", f0=" << f0
        << endl;
    ...
    {
        if (_par.size() != 3) PPPBaseObject::onError(
                PPPSIGNALGEN_ERRPAR + string("evalWindowedModulation()"));
        for (unsigned i = 0; i < aDest.points(); i++)
        {
        ...
        // Notation
        strstream str;
        str << PPPSIGNALGEN_MODULATION << ": time interval = [" << _par[0] << ", " << _par[1] << "]"
    ...
    {
        if (_par.size() != 1) PPPBaseObject::onError(
                PPPSIGNALGEN_ERRPAR + string("addRandomNoise()"));
        double m = _par[0];
        for (unsigned i = 0; i < aDest.points(); i++)
        {
            PPPcomplex r = aDest(i, aNum)
                    + PPPcomplex(m * (double) rand() / RAND_MAX - m / 2.0,
                                 m * (double) rand() / RAND_MAX - m / 2.0);
            PPPBaseTemplate<AType>::cmplConvert(aDest(i, aNum), r);
        }
        // Notation
        strstream str;
        str << PPPSIGNALGEN_RANDNOISE << ": noise value = [" << -m / 2.0 << ", " << m / 2.0 << "]"
        << ends;
        PPPBaseObject::onNotation(str.str());
    }
