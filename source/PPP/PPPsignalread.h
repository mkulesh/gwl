...
public:
    ...
    inline double getSamplingFreq (void) const
    {
        return _sample;
    }
    
    inline void setSamplingFreq (double asample)
    {
        _sample = asample;
    }
    ...
    inline void setFileName (const string &afile)
    {
        _file = afile;
    }
    
    inline void setFileName (const string &afilex, const string &afiley)
    {
        _fileX = afilex;
        _fileY = afiley;
    }
    
    inline double getTmin (void) const
    {
        return _tmin;
    }
    
    inline void setTmin (double atmin)
    {
        _tmin = atmin;
    }
    
    inline double getTmax (void) const
    {
        return _tmax;
    }
    
    inline void setTmax (double atmax)
    {
        _tmax = atmax;
    }
    
    inline bool getToPowerOfTwo (void) const
    {
        return _toPowerOfTwo;
    }
    
    inline void setToPowerOfTwo (bool atoPowerOfTwo)
    {
        _toPowerOfTwo = atoPowerOfTwo;
    }
    
    inline unsigned getGeophone (void) const
    {
        return _geophone;
    }
    
    inline void setGeophone (unsigned ageophone)
    {
        _geophone = ageophone;
    }
    
    inline unsigned getResample (void) const
    {
        return _resample;
    }
    
    inline void setReample (unsigned aresample)
    {
        _resample = aresample;
    }
    
    inline PPPVectorContainer<unsigned> & getChannels (void)
    {
        return _channels;
    }
    
    inline PPPVectorContainer<double> & getRotation (void)
    {
        return _rotation;
    }
    
    inline AType getMult (void) const
    {
        return _mult;
    }
    
    inline void setMult (AType amult)
    {
        _mult = amult;
    }
    
    inline unsigned getBPS (void) const
    {
        return _bps;
    }
    
    inline unsigned getWChannels (void) const
    {
        return _wchannels;
    }
    ...
    void readFunctionASCII (PPPSignalContainer<AType> &aDest, bool aIsTime = true)
    {
        ...
        {
            PPPcomplex z =
                    (PPPBaseTemplate<AType>::isComplex()) ?
                            PPPcomplex(func(i, _channels[0]), func(i, _channels[1])) :
                            PPPcomplex(func(i, _channels[0]), 0.0);
            PPPBaseTemplate<AType>::cmplConvert(aDest(i, 0), z);
        }
        // Notation
        strstream str;
        if (!aIsTime)
            str << PPPSIGNALREAD_TYPE01;
        else
            str << PPPSIGNALREAD_TYPE02;
        if (PPPBaseTemplate<AType>::isReal() && _channels.size() > 0) str << ": "
                                                                      << PPPSIGNALREAD_CHANNAME
                                                                      << "=" << _channels[0];
        if (!PPPBaseTemplate<AType>::isReal() && _channels.size() > 0) str << ": "
                                                                       << PPPSIGNALREAD_CHANNAME
                                                                       << "="
                                                                       << _channels.vectorToStr();
        str << ends;
    ...
    void readFunctionGolm (PPPSignalContainer<AType> &aDest)
    {
        ...
        for (unsigned i = 0; i < aDest.points(); i++)
        {
            PPPcomplex z =
                    (PPPBaseTemplate<AType>::isComplex()) ?
                            PPPcomplex(func(_geophone, i, _channels[0]),
                                       func(_geophone, i, _channels[1])) :
                            PPPcomplex(func(_geophone, i, _channels[0]), 0.0);
            PPPBaseTemplate<AType>::cmplConvert(aDest(i, 0), z);
        }
        // Notation
        strstream str;
        ...
        << ", ";
        if (PPPBaseTemplate<AType>::isReal() && _channels.size() > 0) str << PPPSIGNALREAD_CHANNAME
                                                                      << "=" << _channels[0];
        if (!PPPBaseTemplate<AType>::isReal() && _channels.size() > 0) str << PPPSIGNALREAD_CHANNAME
                                                                       << "="
                                                                       << _channels.vectorToStr();
    ...
    void readFunctionWav (PPPSignalContainer<AType> &aDest)
    {
        ...
        long int ind2 =
                (_tmax == 0) ? infile.points() : (long int) (_tmax * _rate + infile.channels());
        if (ind2 > infile.points()) ind2 = infile.points();
        long int maxind = (long int) powl(2l, (long int) (7 * sizeof(unsigned)));
        if ((ind2 - ind1) > maxind) PPPBaseObject::onError(
                PPPSIGNALREAD_LONG + string("readFunctionWav"));
        ...
        for (unsigned k = 0; k < length; k++, i++)
        {
            ...
            if (infile.channels() == 1)
                z = PPPcomplex(infile.getData(i), 0.0);
            else
            {
                z = PPPcomplex(infile.getData(i), infile.getData(i++));
            }
    ...
    void readSeisASCII (PPPSignalContainer<AType> &aDest, bool aIsTime = true)
    {
        ...
        strstream str;
        if (!aIsTime)
            str << PPPSIGNALREAD_TYPE04;
        else
            str << PPPSIGNALREAD_TYPE05;
        if (_channels.size() > 0) str << ": " << PPPSIGNALREAD_CHANNAME << "="
                                  << _channels.vectorToStr();
        str << ends;
        PPPBaseObject::onNotation(str.str());
    }
    ...
    void readSeisGolm (PPPSignalContainer<AType> &aDest)
    {
        ...
        str << func.getNotation() << endl << "  " << PPPSIGNALREAD_GEONAME << "=" << _geophone
        << ", ";
        if (_channels.size() > 0) str << PPPSIGNALREAD_CHANNAME << "=" << _channels.vectorToStr();
        str << ends;
        PPPBaseObject::onNotation(str.str());
    }
    ...
    void readSeis2Files (PPPSignalContainer<AType> &aDest, bool aIsTime = true)
    {
        ...
        if (sx.channels() != sy.channels()) PPPBaseObject::onError(
                FILE_ERRFORM + string("read2DSeis"));
        ...
        for (unsigned j = 0; j < newcount; j++)
            for (unsigned i = 0; i < sx.points(); i++)
                if (_channels.size() == 0)
                    PPPBaseTemplate<AType>::cmplConvert(aDest(i, j),
                                                        PPPcomplex(sx(i, j), sy(i, j)));
                else
                    PPPBaseTemplate<AType>::cmplConvert(
                            aDest(i, j), PPPcomplex(sx(i, _channels[j]), sy(i, _channels[j])));
        // Notation
        strstream str;
        if (!aIsTime)
            str << PPPSIGNALREAD_TYPE06;
        else
            str << PPPSIGNALREAD_TYPE07;
        if (_channels.size() > 0) str << ": " << PPPSIGNALREAD_CHANNAME << "="
                                  << _channels.vectorToStr();
        str << ends;
        PPPBaseObject::onNotation(str.str());
...
private:
    ...
    void _readPlaneASCII (PPPSpectrContainer<double> &aDest, const string &aName, double aMin = 0.0,
                          double aMax = 0.0)
    {
        strstream stat1;
        ...
        for (j = 0; j < aDest.channels(); j++)
            aDest.getChannel(j).setObjectName("Unknown");
        i = j = k = 0;
        flag1 = 1;
        double val;
        for (unsigned m = 0; m < lines.size(); ++m)
        {
            ...
            if (strncmp(str, tegs[1], strlen(tegs[1])) == 0) aDest.getChannel(j).setObjectName(str);
        }
        strstream stat2;
        stat2 << aDest.getObjectName() << ": " << aDest.voices() << " x ";
        for (j = 0; j < aDest.channels(); j++)
            stat2 << aDest.getChannel(j).getObjectName() << "; ";
        stat2 << ends;
