...
public:
    ...
    inline PPPcomplex CmplPhase (double f)
    {
        ...
        if (aDist == 0.0) onError("Distance is zero in procedure: CmplPhase");
    ...
    {
        double aDist = _params[6];
        if (aDist == 0.0) onError("Distance is zero in procedure: CmplPhasePrim");
        double V1 = _params[0] + _params[1] * exp(-f * f / (_params[2] * _params[2]));
        double V2 = _params[3] + _params[4] * exp(-f * f / (_params[5] * _params[5]));
        double V1p = -2.0 * _params[1] * f * exp(-f * f / (_params[2] * _params[2]))
                / (_params[2] * _params[2]);
        double V2p = -2.0 * _params[4] * f * exp(-f * f / (_params[5] * _params[5]))
                / (_params[5] * _params[5]);
