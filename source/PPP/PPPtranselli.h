...
public:
    ...
    inline PPPConstETmachine getMachine (void)
    {
        return _machine;
    }
    
    void setMachine (PPPConstETmachine amashine)
    {
        _machine = amashine;
    }
    
    /** Direct Elliptic Transform for signal ************************************/
    void eval2DSignalPar (PPPSignalContainer<PPPellipse2D> &aElli,
                          PPPSignalContainer<double> &aVoice, unsigned aN = 1)
    {
        if (aVoice.channels() != 2) onError(PPPTRANSELLI_ERRSOURS + string("eval3DSignalPar()"));
        strstream str;
        ...
        case WTESumCovar:
            str << PPPTRANSELLI_CALCDIR << endl << "  " << getMachine() << ", "
            << PPPSIGNALCONTAINER_NAME << endl << "  " << PPPTRANSELLI_TWIND << "=" << aN << ends;
            onMessage(str.str());
            Zx.resize(aN);
            Zy.resize(aN);
            for (unsigned i = 0; i < aElli.points(); i++)
        ...
        case WTEComplex:
            ...
            for (unsigned i = 0; i < aElli.points(); i++)
                if (aVoice(i) != 0.0) aElli(i).CWTParams(Hx(i), Hy(i), wx(i), wy(i));
            break;
        default:
    ...
    void eval2DSignalPar (PPPSignalContainer<PPPellipse2D> &aFunc,
    ...
    void eval2DSignalPar (PPPSignalContainer<PPPellipse2D> &aElli,
                          PPPSpectrContainer<PPPcomplex> &aWgFull,
                          PPPSignalContainer<double> &aRidge, double aFilter)
    {
        ...
        double maxeng = trans(aWg1.getChannel().getMaxValue(trans))
                + trans(aWg2.getChannel().getMaxValue(trans));
        switch (getMachine())
        {
        case WTEMlinet:
            str << PPPTRANSELLI_CALCDIR << endl << "  " << getMachine() << ", "
            << PPPSPECTRCONTAINER_NAME << endl << "  " << PPPTRANSELLI_FILT << "=" << aFilter << "%"
            << ends;
            onMessage(str.str());
    ...
    void eval2DSignalParInverse (PPPSignalContainer<PPPcomplex> &aVoice,
    ...
    inline PPPcomplex& getCwtLink1 (PPPSpectrContainer<PPPcomplex> &aWg, unsigned i, unsigned j,
                                    unsigned k = 0)
    {
        return (getMachine() == WTEComplex) ? aWg(aWg.voices() / 2 - 1 - i, j, k) : aWg(i, j, 2 * k);
    }
    
    inline PPPcomplex& getCwtLink2 (PPPSpectrContainer<PPPcomplex> &aWg, unsigned i, unsigned j,
                                    unsigned k = 0)
    {
        return (getMachine() == WTEComplex) ? aWg(aWg.voices() / 2 + i, j, k) : aWg(i, j, 2 * k + 1);
    }
    
    void eval2DSpectrPar (PPPSpectrContainer<PPPellipse2D> &aElli,
                          PPPSpectrContainer<PPPcomplex> &aWg, double aFilter = 0.0,
                          unsigned aN = 1, unsigned aChann = 0, bool aDoPrepare = true)
    {
        strstream str;
        ...
        case WTEComplex:
            str << PPPTRANSELLI_CALCDIR << endl << "  " << getMachine() << ", "
            << PPPSPECTRCONTAINER_NAME << ",  " << PPPTRANSELLI_CHANNAME << "=" << aChann << endl
            << "  " << PPPTRANSELLI_FILT << "=" << aFilter << "%" << ends;
            onMessage(str.str());
            if (aDoPrepare)
            {
                aElli.realloc(aWg.voices() / 2, aWg.points());
                aElli.setNames(PPPBASETEMPLATE_EPAR2D, PPPSPECTRCONTAINER_TIME,
        ...
        case WTECovar:
            ...
            onMessage(str.str());
            if (aDoPrepare) aElli.prepare(aWg.voices(), aWg.points(), 1, aWg.getTime(),
                                          aWg.getFreq(), PPPBASETEMPLATE_EPAR2D);
            maxeng = trans(aWg.getChannel(0).getMaxValue(trans))
                    + trans(aWg.getChannel(1).getMaxValue(trans));
    ...
    void eval2DFilter (PPPSpectrContainer<PPPcomplex> &aWgFiltered,
                       PPPSpectrContainer<PPPcomplex> &aWg, PPPSpectrContainer<PPPellipse2D> &aElli,
                       PPPEllipse2Dfilter &aFilter, unsigned aChann, unsigned aSChann = 0)
    {
    ...
    void eval3DSignalPar (PPPSignalContainer<PPPellipse3D> &aElli,
                          PPPSignalContainer<double> &aVoice, unsigned aN = 1)
    {
        if (aVoice.channels() != 3) onError(PPPTRANSELLI_ERRSOURS + string("eval3DSignalPar"));
        ...
        case WTECovar:
            ...
            for (unsigned i = 0; i < aElli.points(); i++)
                aElli(i).CovarianceParams(aH(i, 0), aH(i, 1), aH(i, 2), aW(i, 0), aW(i, 1),
                                          aW(i, 2), aN);
            break;
        default:
    ...
    void eval3DSignalParInverse (PPPSignalContainer<double> &aVoice,
    ...
    void eval3DSpectrPar (PPPSpectrContainer<PPPellipse3D> &aElli,
                          PPPSpectrContainer<PPPcomplex> &aWg, double aFilter = 0.0,
                          unsigned aN = 1)
    {
        if (aWg.channels() != 3) onError(PPPTRANSELLI_ERRSOURS + string("eval3DSpectrPar"));
        aElli.prepare(aWg.voices(), aWg.points(), 1, aWg.getTime(), aWg.getFreq(),
                      PPPBASETEMPLATE_EPAR3D);
        unsigned i, j, k;
        ...
        case WTECovar:
            str << endl << "  " << PPPTRANSELLI_FILT << "=" << aFilter << "%" << endl << "  "
            << PPPTRANSELLI_TWIND << "=" << aN << ends;
            onMessage(str.str());
            ...
            for (i = 0; i < aWg.voices(); i++)
            {
                ...
                for (j = 0; j < aWg.points(); j++)
                {
                    ...
                    curreng = abs(aC[0]) + abs(aC[1]) + abs(aC[2]);
                    if (curreng >= aFilter * maxeng / 100.0)
                        aElli(i, j).CovarianceParams(aC[0], aC[1], aC[2], aW(j, 0), aW(j, 1),
                                                     aW(j, 2), aN);
                    else
                        aElli(i, j) = 0.0;
                }
    ...
    void eval3DSpectrParInverse (PPPSpectrContainer<PPPcomplex> &aWg,
                                 PPPSpectrContainer<PPPellipse3D> &aElli)
    {
        strstream str;
        str << PPPTRANSELLI_CALCINV << endl << "  " << WTEMorozov << ", " << PPPSIGNALCONTAINER_NAME
        << "[3]" << ends;
        onMessage(str.str());
    ...
    void eval3DFilter (PPPSignalContainer<double> &aDest, PPPSignalContainer<double> &aSource,
                       PPPSignalContainer<PPPellipse3D> &aElli)
    {
        if (aSource.channels() != 3) onError(PPPTRANSELLI_ERRSOURS + string("eval3DFilter"));
        ...
        for (unsigned i = 0; i < aElli.points(); i++)
        {
            ...
            for (unsigned k = 0; k < 3; k++)
                if (rmax == 0.0)
                    ...
                    aDest(i, k) = aSource(i, k) * (1.0 - aElli(i).ratio())
                                  * fabs(aElli(i).vecmax(k) / rmax);
        }
    }
    
    /** 3D Notrmal-Ratio Filter *************************************************/
    void eval3DFilter (PPPSpectrContainer<PPPcomplex> &aWgFiltered,
                       PPPSpectrContainer<PPPcomplex> &aWg, PPPSpectrContainer<PPPellipse3D> &aElli,
                       PPPEllipse3Dfilter &aFilter, unsigned aChann)
    {
        ...
        for (i = 0; i < aElli.voices(); i++)
            for (j = 0; j < aElli.points(); j++)
            {
                ...
                {
                    aWgFiltered(i, j, 3 * aChann + 0) = aWg(i, j, 0) * aElli(i, j).planarity_cosx()
                            / (M_PI / 2.0);
                    aWgFiltered(i, j, 3 * aChann + 1) = aWg(i, j, 1) * aElli(i, j).planarity_cosy()
                            / (M_PI / 2.0);
                    aWgFiltered(i, j, 3 * aChann + 2) = aWg(i, j, 2) * aElli(i, j).planarity_cosz()
                            / (M_PI / 2.0);
                }
                ...
                {
                    for (k = 0; k < 3; k++)
                        aWgFiltered(i, j, 3 * aChann + k) = 0.0;
                    numb++;
                }
            }
        strstream str;
        ...
        if (aFilter.getType() != PPPEllipse3Dfilter::NRFNorm)
        {
            ...
            str << endl << "  " << PPPTRANSELLI_DEL << ": " << numb << "/" << cnt << " ("
            << 100.0 * numb / cnt << "%)";
        }
        str << ends;
        onMessage(str.str());
