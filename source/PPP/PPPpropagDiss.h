...
public:
    ...
    typedef enum
    {
        STnone, STfour, STfourcross, STwav, STwavcross
    } PropType;
...
public:
    ...
    inline PPPDispersionModel &getModel ()
    {
        return _model;
    }
    ...
    inline void setDistance (double adX)
    {
        _distance = adX;
    }
                           ...
                           PPPSignalContainer<PPPcomplex> &aSource, double dX1 = 0, double dX2 = 0)
    {
        ...
        {
        case STfour:
            onMessage(PPPPROPAGATORDISS_FOUR);
            dX1 = getDistance();
            dX2 = 0.0;
            break;
        case STfourcross:
            onMessage(PPPPROPAGATORDISS_FOURCR);
            if (dX1 == 0.0 && dX2 == 0.0) dX2 = getDistance();
            break;
        default:
            onError(PPPPROPAGATORDISS_ERRNOTDEF + string("evalFouriePropag"));
            break;
        }
        double f, atnval;
        ...
        for (unsigned i = 0; i < aSource.points() / 2; i++)
        {
            ...
            Sminus(i) = atnval * exp(+2.0 * _j * M_PI * (dX1 - dX2) * getModel().Phi(f))
                        * Sminus(i);
        }
        ...
        if (aPropType == STfourcross)
        {
            ...
            str << "  YS2(f) = exp(-(dX1+dX2)*Atn(f))*exp(-2.0*Pi*i*(dX1-dX2)*Phi(f))*YS1(f)"
            << ends;
        }
        onNotation(str.str());
        return;
    ...
    void evalFourieCrossPropag (PPPSignalContainer<PPPcomplex> &aDest,
                                PPPSignalContainer<PPPcomplex> &aSource,
                                PPPMatrixContainer<unsigned> &aNumb)
    {
        onMessage(PPPPROPAGATORDISS_FOURCRS);
        if (aNumb.cols() != 2) onError(ARG_VALUE + string("evalFourieCrossPropag"));
                            ...
                            unsigned aAlg, double dX1 = 0, double dX2 = 0)
    {
        ...
        {
        case STwav:
            onMessage(PPPPROPAGATORDISS_WAV);
            dX1 = getDistance();
            dX2 = 0.0;
            break;
        case STwavcross:
            onMessage(PPPPROPAGATORDISS_WAVCR);
            if (dX1 == 0.0 && dX2 == 0.0) dX2 = getDistance();
            break;
        default:
            onError(PPPPROPAGATORDISS_ERRNOTDEF + string("evalWaveletPropag"));
            break;
        }
        ...
        if (aCatFlag)
        {
            ...
            for (unsigned i = 0; i < AMinus.voices(); i++)
                AMinus.getFreq()[i] = fabs(AMinus.getFreq(i));
        ...
        if (aAlg == 1) // Standart propagator
        {
                ...
                for (unsigned j = 0; j < ANewPlus.points(); j++)
                {
                    ...
                    ANewPlus(i, j) = atnval
                            * exp(-2.0 * _j
                                  * M_PI
                                  * (dX1 - dX2)
                                  * (PropPar(i, PPPDispersionModel::iWn) - f
                                          * PropPar(i, PPPDispersionModel::iWndiv)))
                            * APlus.Get(f,
                                        b - (dX1 - dX2) * PropPar(i, PPPDispersionModel::iWndiv));
                    if (aTransPar.getFreq().getSign() == PPPAxis::ASfull) ANewMinus(i, j) = atnval
                            * exp(+2.0 * _j
                                  * M_PI
                                  * (dX1 - dX2)
                                  * (PropPar(i, PPPDispersionModel::iWn) - f
            ...
            if (aPropType == STwav)
            {
                ...
                str << "  WgS2(t,f) = exp(-dX*Atn(f))*exp(-2.0*Pi*i*dX*(Phi(f)-f*Phidiv(f)))*WgS1(t-dX*Phidiv(f),f)"
                << ends;
            }
            if (aPropType == STwavcross)
            {
                str << PPPPROPAGATORDISS_WAVCR << endl;
                str << "  WgS2(t,f) = exp(-(dX+dX2)*Atn(f))*exp(-2.0*Pi*i*(dX1-dX2)*(Phi(f)-f*Phidiv(f)))*WgS1(t-(dX1-dX2)*Phidiv(f),f)"
                << ends;
            }
        }
        else if (aAlg == 2) // Modulus-Argument propagator
        {
            for (unsigned i = 0; i < ANewPlus.voices(); i++)
                for (unsigned j = 0; j < ANewPlus.points(); j++)
                {
                    ...
                    double atnval = exp(-(dX1 + dX2) * PropPar(i, PPPDispersionModel::iAtn));
                    double zp1 = abs(
                            APlus.Get(f, b - (dX1 - dX2) / PropPar(i, PPPDispersionModel::iCg)));
                    double zp2 = arg(
                            APlus.Get(f, b - (dX1 - dX2) / PropPar(i, PPPDispersionModel::iCp))); // in general -1.0/f
                    double zm1 = abs(
                            AMinus.Get(f, b - (dX1 - dX2) / PropPar(i, PPPDispersionModel::iCg)));
                    double zm2 = arg(
                            AMinus.Get(f, b - (dX1 - dX2) / PropPar(i, PPPDispersionModel::iCp))); // in general -1.0/f
            ...
            {
                str << PPPPROPAGATORDISS_WAV << endl;
                str << "  WgS2(t,f) = exp(-dX*Atn(f))*|WgS1(t-dX/VSg(f),f)|*exp[i arg WgS1(t-dX/VSp(f),f)]"
                << ends;
            }
            if (aPropType == STwavcross)
            {
                str << PPPPROPAGATORDISS_WAVCR << endl;
                str << "  WgS2(t,f) = exp(-(dX1+dX2)*Atn(f))*|WgS1(t-(dX1-dX2)/VSg(f),f)|*exp[i arg WgS1(t-(dX1-dX2)/VSp(f),f)]"
                << ends;
            }
        }
            ...
            for (unsigned i = 0; i < ANewPlus.voices(); i++)
            {
                ...
                phasep = -2.0
                        ...
                        * (PropPar(i, PPPDispersionModel::iWn) - f
                                * PropPar(i, PPPDispersionModel::iWndiv))
                        * (dX1 - dX2)
                         - (PropPar(i, PPPDispersionModel::iAtn) - f
                                 * PropPar(i, PPPDispersionModel::iAtndiv))
                           * (dX1 + dX2);
                phasem = +2.0
                        * M_PI
                        * _j
                        ...
                        * (dX1 - dX2)
                         - (PropPar(i, PPPDispersionModel::iAtn) - f
                           ...
                           * (dX1 + dX2);
                double fs = 1.0
                        + (dX1 + dX2) * f * PropPar(i, PPPDispersionModel::iAtndiv) / (aTransPar
                                .getWaveletPar(PPPSpectrParams::WSTdirect));
        ...
        else if (aAlg == 4) // Classic elliptic propagator
        {
            ...
            for (unsigned i = 0; i < ANewPlus.voices(); i++)
                for (unsigned j = 0; j < ANewPlus.points(); j++)
                {
                    ...
                    ew.CWTParamsDefomDiss(
                            exp(-dX * PropPar(i, PPPDispersionModel::iAtn)),
                            2.0 * M_PI
                            * dX
                            * (PropPar(i, PPPDispersionModel::iWn) - f
                                    * PropPar(i, PPPDispersionModel::iWndiv)));
        ...
        if (aCatFlag)
            _trans.WaveletCat(aDest, ANewPlus, ANewMinus);
        else
            aDest.assign(ANewPlus);
        onNotation(str.str());
        return;
    }
