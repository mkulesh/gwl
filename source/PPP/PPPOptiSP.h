...
public:
    ...
    PPPOptiSpectrum () :
            _cmplType(TCabs),
            _propType(PPPPropagatorDiss::STwav)
    {
    ...
    void prepare (PPPSpectrContainer<PPPcomplex> &aSource, PPPDispersionModel &aModel, double aDist,
                  ...
                  unsigned aCmplType, unsigned aChan1 = 0, unsigned aChan2 = 1)
    {
        // wavelet spectrum
        _wg1.prepare(aSource.voices(), aSource.points(), 1, aSource.getTime(), aSource.getFreq(),
        PPPOPTISPECTRUM_SPEC1NAME);
        _wg1.getChannel(0).assign(aSource.getChannel(aChan1));
        _wg2.prepare(aSource.voices(), aSource.points(), 1, aSource.getTime(), aSource.getFreq(),
        PPPOPTISPECTRUM_SPEC2NAME);
        ...
        ATypeOpti::evalFixed((getWn().getType() == PPPApproximate::APTpolin), (_cmplType == TCarg),
                             getWn().size());
    }
    ...
    void calcfunc (PPPVectorContainer<double> &aPar)
    {
        ...
        for (unsigned j = 0; j < ATypeOpti::points(); j++)
        {
            ...
            df = (_indj > 0) ?
                    (getComponent(_wgn(_indi, _indj)) - getComponent(_wgn(_indi, _indj - 1))) : 0.0;
            ...
            for (unsigned m = 0; m < ATypeOpti::params(); m++)
            {
                der1 = (m < getWn().size()) ?
                        ...
                        -dX * getAtn().FuncDivPar(f, m - getWn().size())
                        * getComponent(_wgn(_indi, _indj));
                _der(m, j) = der1;
            }
        }
        return;
...
private:
    ...
    double getComponent (PPPcomplex const &aVal)
    {
        ...
        case TCarg:
            return arg(aVal);
        default:
            PPPBaseObject::onError(PPPBASETEMPLATE_ERR + string("getComponent"));
        }
        return 0;
    }
