...
class PPPWavIn : public PPPBaseObject
  ...
  protected:
     ...
     /*
     1) Для 8-битовых данных диапазон изменения составляет от 0 до 255 (Охff),
     причем отсутствию сигнала (полной тишине) соответствует значение 128 (0х80).
     Для монофонического сигнала звуковые данные представляют собой массив
     однобайтовых значений, каждое из которых является выборкой сигнала.
     Для стереофонического сигнала звуковые данных имеют формат массива
     двухбайтовых слов, причем младший байт слова соответствует левому
     каналу, а старший - правому.

     2) Для 16-битовых данных диапазон изменения составляет от -32768 (-0х8000)
     до 32767, (Ox7fff), отсутствию сигнала соответствует значение 0.
     Для монофонического сигнала данные хранятся в массиве 16-битовых слов.
     Для стереофонического используется массив двойных слов, причем младшему
     слову соответствует левый канал, а старшему - правый.
     */
  ...
  public:
    ...
    PPPWavIn(const string &aName) : _data(NULL), _points(0), _bps(0), _channels(0), _rate(0) {
      ...
      if(strcmp(obuff,"RIFF")!=0)
        onError(PPPWAVIN_ERRFOR + string("RIFF"));

      for(i=0;i<4;i++) obuff[i] = wav->wID[i];
      obuff[4] = 0;
      if(strcmp(obuff,"WAVE")!=0)
        onError(PPPWAVIN_ERRFOR + string("WAVE"));

      for(i=0;i<3;i++) obuff[i] = wav->fId[i];
      obuff[3] = 0;
      if(strcmp(obuff,"fmt")!=0)
        onError(PPPWAVIN_ERRFOR + string("FMT"));

      if(wav->wFormatTag!=1)
        onError(PPPWAVIN_ERRFOR + string("wFormatTag"));

      if( (wav->nBitsPerSample != 16) && (wav->nBitsPerSample != 8) )
        onError(PPPWAVIN_ERRFOR + string("BPS"));
      ...
      if(wav->nBitsPerSample == 16)
        {
        uptr = (short *) wbuff;
        for(i=0;i<points();i++) _data[i] = (int) (uptr[i]);
        }
      else
        {
        cptr = (unsigned char *) wbuff;
        for(i=0;i<points();i++) _data[i] = (int) (cptr[i]) - 0x80;
        }
    ...
    inline int getData(long int aInd) const {
     ...
     inline long int points() const { return _points; };
     ...
     inline unsigned getBPS() const { return _bps; };

     // reports sample rate in Hz
     inline double getRate() const { return _rate; };
