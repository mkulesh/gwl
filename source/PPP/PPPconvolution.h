...
public:
    ...
    PPPConvolutorSig (int an0, int an1) :
            _type(DTnone),
            _n0(an0),
            _n1(an1)
    {
    ...
    inline PPPVectorContainer<AType> & getSignal ()
    {
        return _sig;
    }
    ...
    {
        if (vec.size() != _sigview.size()) PPPBaseObject::onError(
                PPPCONVOLUTORSIG_ERR2 + string("setSignal()"));
        for (unsigned i = 0; i < vec.size(); i++)
            _sigview[i] = aCoeff * vec[i];
        finish();
    }
    
    void finish ()
    {
        if (_n0 == 0 && _n1 == 0) PPPBaseObject::onError(
                PPPCONVOLUTORSIG_ERR1 + string("finish()"));
...
public:
     ...
     */
    PPPConvolutor (int & nsig0, // (in) lower index of first signal (out) actually needed lower index
            ...
            unsigned aSigCount = 1, unsigned aSagCount = 1)
    {
        PPPBaseObject::setObjectName(PPPCONVOLUTOR_NAME);
        int nn = _evalIndexies(nsig0, nsig1, nsag0, nsag1, nsug0, nsug1, unknownLimitSig,
                               unknownLimitSag);
        // first signal
        _sig.resize(aSigCount);
        for (unsigned i = 0; i < getSigSize(); i++)
        {
            if ((_sig[i] = new PPPConvolutorSig<AType>(nsig0, nsig1)) == NULL) PPPBaseObject::onError(
                    MEM_ERRALLOC + string("PPPConvolutor"));
        ...
        {
            if ((_sag[i] = new PPPConvolutorSig<AType>(nsag0, nsag1)) == NULL) PPPBaseObject::onError(
                    MEM_ERRALLOC + string("PPPConvolutor"));
        ...
        {
            if ((_sug[i] = new PPPConvolutorSig<AType>(nsug0, nsug1)) == NULL) PPPBaseObject::onError(
                    MEM_ERRALLOC + string("PPPConvolutor"));
...
#endif
        int _s0 = max(_sig[aInd1]->getFirstIndex() + _sag[aInd2]->getFirstIndex(),
                      _sug[aInd]->getFirstIndex());
        int _s1 = min(_sig[aInd1]->getLastIndex() + _sag[aInd2]->getLastIndex(),
                      _sug[aInd]->getLastIndex());
        if (aZero)
        {
            for (int i = 0; i < _s0 - _sug[aInd]->getFirstIndex(); ++i)
                aDest[i] = 0;
            for (int i = _s1 - _sug[aInd]->getFirstIndex() + 1;
                    i < _sug[aInd]->getFirstIndex() + _sug[aInd]->getLastIndex(); ++i)
                aDest[i] = 0;
        }
        PPPVectorContainer<PPPcomplex> tmp;
        tmp.link(aDest, _s0 - _sug[aInd]->getFirstIndex(), _s1 - _s0 + 1);
        tmp.assign(_sug[aInd]->getSignal(),
                   _s0 - (_sig[aInd1]->getFirstIndex() + _sag[aInd2]->getFirstIndex()),
                   _s1 - _s0 + 1);
        tmp.unlink();
    }
...
private:
    ...
    int _evalIndexies (int & nsig0, // (in) lower index of first signal (out) actually needed lower index
            int & nsig1,  // (in) upper index of first signal (out) actually needed upper index
            int & nsag0,  // (in) lower index of second signal (out) actually needed lower index
            int & nsag1,  // (in) upper index of second signal (out) actually needed upper index
            int & nsug0, // (in) lower index of result signal (the actual lower limit is nsig0+nsag0)
            ...
            )
    {
        ...
        if ((nsig0 + nsag0 > nsug1) && (nsig1 + nsag1 < nsug0)) PPPBaseObject::onError(
                PPPCONVOLUTOR_ERR2);
        ...
        nsag0 = nnsag0;
        nsag1 = nnsag1;
        if ((nsig0 + nsag0 > nsug1) | (nsig1 + nsag1 < nsug0) | (nsig0 > nsig1) | (nsag0 > nsag1)) PPPBaseObject::onError(
                PPPCONVOLUTOR_ERR2);
