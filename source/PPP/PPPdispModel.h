...
public:
    ...
    PPPDispersionModel (void) :
            ...
            _freqmin(0),
            _freqmax(0),
            _intrp(PPPSignalContainer<double>::ITsplin),
            _isAnalytical(true)
    {
    ...
    {
        strstream aDest;
        aDest << getObjectName() << endl << "  " << PPPDISPERSIONMODEL_FREQ << ": [" << getFrMin()
        << ", " << getFrMax() << "]";
        if (isAnalytical())
        {
    ...
    inline bool isAnalytical (void) const
    {
        return (_isAnalytical);
    }
    
    void setAnalytical (bool const aAnalytical)
    {
        _isAnalytical = aAnalytical;
    }
                  ...
                  PPPApproximate::ApprType aAtnType, PPPVectorContainer<double> &aAtn)
    {
        ...
        switch (aWnType)
        {
        case PPPApproximate::APTvel:
            _wn = _createApprox(1, aWnSize);
            break;
        case PPPApproximate::APTgauss:
            _wn = _createApprox(2, aWnSize);
            break;
        case PPPApproximate::APTpolin:
            _wn = _createApprox(3, aWnSize);
            break;
        case PPPApproximate::APTbspline:
            _wn = _createApprox(4, aWnSize);
            break;
        case PPPApproximate::APTcolecole:
            _wn = _createApprox(5, aWnSize);
            break;
        case PPPApproximate::APTtwogauss:
            _wn = _createApprox(7, aWnSize);
            break;
        default:
            onError(PPPDISPERSIONMODEL_ERRTYPE + string("prepare()"));
        }
        switch (aAtnType)
        {
        case PPPApproximate::APTvel:
            _atn = _createApprox(1, aAtnSize);
            break;
        case PPPApproximate::APTgauss:
            _atn = _createApprox(2, aAtnSize);
            break;
        case PPPApproximate::APTpolin:
            _atn = _createApprox(3, aAtnSize);
            break;
        case PPPApproximate::APTbspline:
            _atn = _createApprox(4, aAtnSize);
            break;
        case PPPApproximate::APTcolecole:
            _atn = _createApprox(6, aAtnSize);
            break;
        case PPPApproximate::APTtwogauss:
            _atn = _createApprox(8, aAtnSize);
            break;
        default:
            onError(PPPDISPERSIONMODEL_ERRTYPE + string("prepare()"));
        }
    ...
    {
        prepare(aSource.getWn().getType(), aSource.getWn().getParams(), aSource.getAtn().getType(),
                aSource.getAtn().getParams());
    ...
    {
        for (unsigned i = 0; i < getWn().size(); i++)
            getWn()[i] = aSource[i];
        for (unsigned i = 0; i < getAtn().size(); i++)
            getAtn()[i] = aSource[getWn().size() + i];
    }
    ...
    inline double getFrMin (void) const
    {
        return _freqmin;
    }
    
    inline double getFrMax (void) const
    {
        return _freqmax;
    }
    ...
    inline double Phi (double f)
    {
        return (isAnalytical()) ? getWn().Func(f) : _data.Get(f, iWn, _intrp);
    }
    
    inline double Phidiv (double f)
    {
        return (isAnalytical()) ? getWn().FuncDivX(f) : _data.Get(f, iWndiv, _intrp);
    }
    ...
    inline double Atn (double f)
    {
        return (isAnalytical()) ? getAtn().Func(f) : _data.Get(f, iAtn, _intrp);
    }
    
    inline double Atndiv (double f)
    {
        return (isAnalytical()) ? getAtn().FuncDivX(f) : _data.Get(f, iAtndiv, _intrp);
    }
    
    inline bool isCausal (void)
    {
        return _isCausal;
    }
    
    // velocities
    inline double VSp (double f)
    {
        return f / fabs(Phi(f));
    }
    
    inline double VSg (double f)
    {
        return 1.0 / fabs(Phidiv(f));
    }
    ...
    void fwrite (FILE *stream)
    {
        ...
        unsigned aT;
        aT = isAnalytical();
        std::fwrite((void*) &aT, sizeof(unsigned), 1, stream);
        if (isAnalytical())
        {
            aT = getWn().getType();
            std::fwrite((void*) &aT, sizeof(unsigned), 1, stream);
...
private:
    ...
    PPPApproximate *_createApprox (unsigned const aWnType, unsigned const aSize)
    {
        ...
        {
        case 1:
            appr = new PPPApproximateVel(aSize);
            break;
        case 2:
            appr = new PPPApproximateGauss(aSize);
            break;
        case 3:
            appr = new PPPApproximatePolinom(aSize);
            break;
        case 4:
            appr = new PPPApproximateBspline(aSize);
            break;
        case 5:
            appr = new PPPApproximateColeColePhi(aSize);
            break;
        case 6:
            appr = new PPPApproximateColeColeAtn(aSize);
            break;
        case 7:
            appr = new PPPApproximateTwoGaussPhi(aSize);
            break;
        case 8:
            appr = new PPPApproximateTwoGaussAtn(aSize);
            break;
        default:
            onError(PPPDISPERSIONMODEL_ERRTYPE + string("createApprox"));
        }
        if (appr == NULL) onError(MEM_ERRALLOC + string("createWavelet"));
        return appr;
    }
    
    inline bool isWn (void)
    {
        return (_wn != NULL);
    }
    
    inline bool isAtn (void)
    {
        return (_atn != NULL);
    }
