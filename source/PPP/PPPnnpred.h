...
public:
                         ...
                         unsigned aL = 2, double aKoeff = 1.0, double aSub = 0.0)
    {
        ...
        if (aL == 3)
            ...
            setObjectName(PPPNNPRED_NAME);
        strstream str;
        str << endl << "  Size of source time serie = " << leng << endl
        << "  Count of predicted points = " << aPredcount << endl
        << "  Count of nearest neighbors = " << aNNcount << endl << "  Embedding dimension = "
        << _dim << endl << "  Time delay = " << _step << endl << "  Norm L" << aL << endl
        ...
        for (unsigned i = 0; i < aPredcount; i++)
        {
            ...
            if (i > 0)
            {
                sprintf(numpoint, "Point %3d, norm values=%s (%d%%)", i + 1, errvals.vectorToStr(),
                        100 * i / (aPredcount - 1));
                onMessage(numpoint);
            }
        }
    ...
    {
        if (aSig.size() == 0) onError("lenght of input signal is 0 in procedure: extractTrend()");
        if (aSize == 0) onError("degree of trend is 0 in procedure: extractTrend()");
        unsigned j, k;
        ...
        for (k = 0; k < _trendCoeff.size(); k++)
            _trendCoeff[k] = P[k];
        if (aMod) for (k = 0; k < aSig.size(); k++)
            aSig[k] -= approximateTrend(k);
        strstream str;
    ...
    {
        double sum = 0;
        for (unsigned k = 0; k < _trendCoeff.size(); k++)
            sum += _trendCoeff[k] * pow(aVal, (double) k);
        return sum;
    }
    
    void restoreTend (PPPVectorContainer<double> &aSig, int aStart = 0)
    {
        for (unsigned k = 0; k < aSig.size(); k++)
            aSig[k] += approximateTrend(k + aStart);
    }
...
private:
    ...
    double _predictPoint (unsigned aNNcount, unsigned aL, PPPVectorContainer<double> &aErrVals)
    {
        ...
        if (_minElem == 0) onError("minimal element of serie is null in procedure: predictPoint()");
    ...
    void _getCandidate (PPPVectorContainer<double> &aCand, unsigned aInd)
    {
        if (aInd < _dim * _step)
        {
            ...
            << "  the embedding dimension=" << _dim << "; the time delay=" << _step << "; index="
            << aInd << endl;
            onError(str.str());
        }
        aCand.resize(_dim);
        for (unsigned i = 0; i < _dim; i++)
            aCand[i] = _data[aInd - _step * i - 1];
    }
    
    double _getDistance (PPPVectorContainer<double> &aVec1, PPPVectorContainer<double> &aVec2,
                         int aL, double &aData, double &aPred)
    {
        if (aVec1.size() != aVec2.size())
        {
            strstream str;
            str << "Invalid dimensions of vectors in procedure getDistance()" << endl
            << "  length of 1th vector=" << aVec1.size() << "; length of 2th vector="
            << aVec2.size() << endl;
            onError(str.str());
        }
        double summ = 0;
        ...
        {
            for (i = 0; i < _dim; i++)
                summ += (aVec1[i] - aVec2[i]) * (aVec1[i] - aVec2[i]);
            aPred = aData;
            return sqrt(summ);
        }
        ...
        if (aL == 3)
        {
            ...
            for (k = 0; k < aLamParts + 1; k++)
            {
                SumByLam = 0;
                for (i = 0; i < _dim; i++)
                    M[i] = aVec1[i] - k / Prec * aVec2[i];
                Med = med_kth_smallest(M, MedInd);
                for (i = 0; i < _dim; i++)
                    SumByLam += fabs(aVec1[i] - k / Prec * aVec2[i] - Med);
