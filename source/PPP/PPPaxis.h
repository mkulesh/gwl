...
public:
    ...
    PPPAxis (unsigned const aSize, const double aMin, const double aMax, const AxisType aAxistype,
             string const &aName)
    {
        _setdefault();
        setObjectName(aName);
    ...
    inline AxisType getType (void) const
    {
        return _axistype;
    }
    
    inline AxisSign getSign () const
    {
        return _axissign;
    }
    
    inline double getSamplingPeriod (void) const
    {
        return _delta;
    }
    ...
    void setParams (const AxisType aAxistype)
    {
        ...
        case ATnone:
        case ATlin:
            _delta = (*this)[1] - (*this)[0];
            break;
        case ATlog:
            _delta = log(_max / _min) / (log(2.0) * (double) (size() - 1));
            break;
        case ATloge:
            _delta = log(_max / _min) / (M_E - 1.0);
            break;
        default:
    ...
    void fill (const double aMin, const double aMax, const AxisType aAxistype)
    {
        ...
        case ATlin:
            ...
            break;
        case ATlog:
            if (aMin == 0.0 || aMax == 0.0) onError(PPPAXIS_ERRVAL + string("fill()"));
    ...
    void fillsign (const AxisSign aType)
    {
        ...
        case ASplus:
            break;
        case ASminus:
            for (unsigned i = 0; i < aAxis.size(); i++)
                (*this)[i] = -aAxis[aAxis.size() - 1 - i];
            _min = -aAxis.getMax();
            _max = -aAxis.getMin();
            break;
        case ASfull:
    ...
    const char *getInfo (void)
    {
        ...
        aDest << " min=" << getMin() << ", max=" << getMax() << ", sampling period="
        << getSamplingPeriod() << ", sampling frequency=" << getSamplingFreq();
    ...
    void fread (FILE *stream)
    {
        ...
        unsigned val;
        std::fread((void*) &val, sizeof(unsigned), 1, stream);
        _axistype = (AxisType) val;
        std::fread((void*) &val, sizeof(val), 1, stream);
        _axissign = (AxisSign) val;
