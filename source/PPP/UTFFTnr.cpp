...
class UTFFT::FFTDATA {
  ...
  public:
    ...
    FFTDATA () : _zeroval(0.0), _fastBits(16) {
    ...
    int fastReverseBits(int index, int aBits) {
      if (aBits <= _fastBits)  return _bitTable[aBits-1][index];
      else  return reverseBits(index, aBits);
      };
...
bool UTFFT::execute(void) {
  ...
  double angle_numerator = (_transtype==C2CBACKWARD || _transtype==C2R)? 2.0*M_PI : -2.0*M_PI;
