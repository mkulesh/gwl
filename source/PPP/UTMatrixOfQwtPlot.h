...
public:
	...
	void
	setRowAxisRange ( unsigned row, double ymin, double ymax )
	{
	...
	void
	setColumnAxisRange ( unsigned col, double xmin, double xmax )
	{
...
void
UTMatrixOfQwtPlot::allignAllPlots (void) {
	...
	for (unsigned c=0; c < _cols; ++c) 
	{
		...
		plot(_rows-1, c)->axisScaleDraw(QwtPlot::xBottom)-> enableComponent(QwtAbstractScaleDraw::Labels, false);
	}
	...
	for (unsigned r=0; r< _rows; ++r)
	{
		...
		(
			width, 
			plot(r,0) -> axisScaleDraw(QwtPlot::yLeft)->extent (pen, font)
		);
	}
	width += (int) (0.1*width);
	for (unsigned r=0; r< _rows; ++r)
