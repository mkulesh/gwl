  ...
  public:
    ...
    void write(const string &aFile, const string &aSource, int aFormat) {
      ...
      if(aFormat == 2 || aFormat == 3)
        ...
        char *str,*str1;
        if((str = new char[buffsize]) == NULL)
          onError(MEM_ERRALLOC+string("write"));
        if((str1 = new char[buffsize]) == NULL)
          onError(MEM_ERRALLOC+string("writewrite"));
        istream *tmp;
    ...
    void onProgress(const int aPercent, const string &aNotation)  {
      if(_messageMode == 0) return;
      if(aPercent<0) { printf("\n"); return; }
      if(aPercent==0) { printf("  %s\r",aNotation.c_str()); return; }
      if(aPercent>0)
        {
        if(aNotation.size()>0) printf("  %s (%d%%)\r",aNotation.c_str(),aPercent);
        else printf("  %d%%\r",aPercent);
        }
      return;
      };
