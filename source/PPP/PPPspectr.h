...
class PPPSpectrContainer : public PPPBaseTemplate<AType>
  ...
  public:
    ...
    typedef enum {ITnone, ITlin} InterpType;
  ...
  public:
    ...
    PPPSpectrContainer(const unsigned aVoices, const unsigned aPoints, const unsigned aChannels=1) {
      PPPBaseObject::setObjectVer(PPPSPECTRCONTAINER_OBJVER);
      setNames(PPPSPECTRCONTAINER_NAME,PPPSPECTRCONTAINER_TIME,PPPSPECTRCONTAINER_FREQ);
    ...
    inline AType& operator() (const unsigned aVoice, const unsigned aPoint, const unsigned aChannel=0) const {
      #ifdef PPPCONF_CHECKINDEX
      _checkindex(aChannel);
      #endif
    ...
    void prepare(PPPSpectrContainer<AType1> & aSour) {
      setNames(aSour.getObjectName(),aSour.getTime().getObjectName(),aSour.getFreq().getObjectName());
    ...
    void assign(PPPSpectrContainer & aSour,
      unsigned aVoiStart=0, unsigned aVoices=0, int aVoiOffset=1,
      unsigned aPoiStart=0, unsigned aPoints=0, int aPoiOffset=1
      ) {
      setNames(aSour.getObjectName(),aSour.getTime().getObjectName(),aSour.getFreq().getObjectName());
      ...
      for(unsigned i=0; i<channels(); i++)
        getChannel(i).assign(
          aSour.getChannel(i),
          aVoiStart, aVoices, aVoiOffset,
          aPoiStart, aPoints, aPoiOffset
          );
      return;
      };
    ...
    const char *getInfo(void) {
      strstream aDest;
      if(channels() == 1)
        aDest<<PPPBaseObject::getObjectName()<<": "<<PPPBaseTemplate<AType>::getTypeName()<<"["<<voices()<<"]["<<points()<<"]"<<endl;
      else
        aDest<<PPPBaseObject::getObjectName()<<": "<<PPPBaseTemplate<AType>::getTypeName()<<"["<<channels()<<"]["<<voices()<<"]["<<points()<<"]"<<endl;
    ...
    AType Get(double aY, double aX, const unsigned aChannel=0, InterpType aC=ITlin)
      {
      if(aX<getTime().getMin() || aX>getTime().getMax() || aY<getFreq().getMin() || aY>getFreq().getMax()) return 0.0;
      ...
      if(aC == ITnone)
        ...
        return ((1-T)*(1-U)*(*this)(i1,j1,aChannel) +
                T*(1-U)*(*this)(i1+1,j1,aChannel) +
                T*U*(*this)(i1+1,j1+1,aChannel) +
                (1-T)*U*(*this)(i1,j1+1,aChannel));
        }
      else PPPBaseObject::onError(PPPSPECTRCONTAINER_ERRINTERP+string("Get"));
      return 0.0;
      };
