...
public:
    ...
    void resize (const unsigned aRowscount, const unsigned aColscount)
    {
        ...
        {
            if ((_data = new AType[aSize]) == NULL) PPPBaseTemplate<AType>::onError(
        ...
        {
            AType *values1;
            if ((values1 = new AType[aSize]) == NULL) PPPBaseTemplate<AType>::onError(
                    MEM_ERRALLOC + string("resize"));
            for (i = 0; i < aRowscount; i++)
                for (j = 0; j < aColscount; j++)
                {
                    if (i < rows() && j < cols())
                        values1[i * aColscount + j] = _data[i * cols() + j];
                    else
                        values1[i * aColscount + j] = PPPBaseTemplate<AType>::nullValue();
                }
            delete _data;
            _data = values1;
            _rowscount = aRowscount;
    ...
    void assign (const PPPMatrixContainer &aSour, unsigned aRowStart = 0, unsigned aRows = 0,
                 int aRowOffset = 1, unsigned aColStart = 0, unsigned aCols = 0, int aColOffset = 1)
    {
    ...
    template<class TransType>
    void write (string const &aName, const unsigned aRowscount, const unsigned aColscount,
                TransType &aTrans, MatrixDir const aDirect = MDvert, int indj1 = -1, int indj2 = -1,
                int indi1 = -1, int indi2 = -1)
    {
        strstream str;
        remove(aName.c_str());
