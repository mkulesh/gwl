...
class gwlNameOptions {
  ...
  public:
    gwlNameOptions (const char *aname, const char *adefault): _name(aname), _default(adefault) {
...
template<class ATypeSource, class ATypeDest> class gwlMainObject : public PPPBaseObject
  ...
  public:
    ...
    gwlMainObject(const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
      ...
      o_outtype("t",  "outtype", "<unsigned>", "type of output: 1-ASCII, 2-binary (by default 2)", 2),
      o_wavelet("w",  "wavelet", "<str>",  "wavelet name: haar, morlet, morletre, cauchy, shanon (by default 'morlet')", "morlet"),
    ...
    void parseFileNames(void) {
      ...
      while(1)
        ...
        if(pos >= aPar.size()) { aFileNames.push_back(aPar);  break; }
        aFileNames.push_back(aPar.substr(0,pos));
        aPar.erase(0,pos+1);
        }
      if(aFileNames.size()<2)
        PPPBaseObject :: onError("too few input signals are given");
      };
    ...
    void write_bindata(AType &aData, const char *aName) {
      ...
      if(strcmp(aData.getObjectVer(),PPPSPECTRCONTAINER_OBJVER) == 0)
        aSpectrumPar.fwrite(outfile);
      fclose(outfile);
      };
    ...
    void evaluate(void) {
      ...
      calc();
      if(o_outfile.isParsed() && strlen(o_outfile.getValue())>0)
        write();
      };
