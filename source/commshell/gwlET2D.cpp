...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_type("y",
                   "type",
                   "<str>",
                   "type of the elliptic mashine: rene, morozov, scovar, acovar, complex, mlinet, mlinef (by default 'complex')",
                   "complex"),
            o_tw("w", "tw", "<unsigned>",
            ...
            o_filter("f", "filter", "<real>", "energy filter by spectral transform (by default 0)",
                     0.0),
            o_freq("q", "freq", "<str>",
                   "frequency interval for averaging's method (by default '')", ""),
    ...
    {
        if (aLocSource.channels() != 2) PPPBaseObject::onError(gwlET2D_ERR01);
    ...
    void calc (PPPSignalContainer<PPPellipse2D> &aLocDest,
    ...
    void calc (PPPSpectrContainer<PPPellipse2D> &aLocDest,
               PPPSpectrContainer<PPPcomplex> &aLocSource)
    {
        if (aTrans.getMachine() == WTERene && ATypeMain::aSpectrumPar.getFreq().getSign()
                != PPPAxis::ASplus
            && aLocSource.channels() != 2) PPPBaseObject::onError(gwlET2D_ERR01);
        if (aTrans.getMachine() == WTEComplex && ATypeMain::aSpectrumPar.getFreq().getSign()
                != PPPAxis::ASfull) PPPBaseObject::onError(gwlET2D_ERR01);
        if (aTrans.getMachine() == WTECovar && ATypeMain::aSpectrumPar.getFreq().getSign()
                != PPPAxis::ASplus
            && aLocSource.channels() != 2) PPPBaseObject::onError(gwlET2D_ERR01);
        aTrans.eval2DSpectrPar(aLocDest, aLocSource, o_filter.getValue(), o_tw.getValue(),
                               o_channel.getValue());
    }
    ...
    void calc (PPPSignalContainer<PPPellipse2D> &aLocDest,
               PPPSpectrContainer<PPPellipse2D> &aLocSource)
    {
        PPPVectorContainer<double> freq;
        freq.strToVector("{" + string(o_freq.getValue()) + "}");
        if (freq.size() != 2) PPPBaseObject::onError(gwlET2D_ERR02);
