...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_sig("s", "sig", "<file>", "input signal's file name (by default 'signal.dat')",
                  "signal.dat"),
            o_osig("z", "osig", "<file>",
                   "optimized signal's file name (by default 'signalopt.dat')", "signalopt.dat"),
    ...
    void calc (void)
    {
        ...
        if (o_chan.isOptionGiven())
        {
            ...
            opti.prepare(aSignal, ATypeMain::aSource, o_dist.getValue(), aPropType, chan[0],
                         chan[1]);
            ...
            if (o_osig.isOptionGiven()) ATypeMain::write_bindata(opti.getResult(),
                                                                 o_osig.getValue());
        }
        else
        {
            ...
            str2 << opti.getInfo();
            ATypeMain::aDest.assign(opti.getModel());
            if (o_osig.isOptionGiven()) ATypeMain::write_bindata(opti.getResultSig(),
                                                                 o_osig.getValue());
        }
