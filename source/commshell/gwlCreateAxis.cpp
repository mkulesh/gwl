...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_scale("s", "scale", "<str>",
                    "scale of the axis: 'lin'-linear, 'log'-logarithmic (by default 'lin')", "lin"),
            o_sign("g",
                   "sign",
                   "<str>",
                   "symmetry of the axis: 'prog'-progressive, 'reg'-regressive, 'full'-full (by default 'prog')",
                   "prog"),
    ...
    void calc (void)
    {
        ...
        if (scaleMap.find(o_scale.getValue()) == scaleMap.end()) PPPBaseObject::onError(
                "scale of the axis is incorrect");
        
        map<string, PPPAxis::AxisSign> signMap;
        signMap["prog"] = PPPAxis::ASplus;
        signMap["reg"] = PPPAxis::ASminus;
        signMap["full"] = PPPAxis::ASfull;
        if (signMap.find(o_sign.getValue()) == signMap.end()) PPPBaseObject::onError(
                "symmetry of the axis is incorrect");
        ...
        if (signMap[o_sign.getValue()] != PPPAxis::ASplus) aDest.fillsign(
                signMap[o_sign.getValue()]);
