...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_name("n", "name", "<str>", "name of the axis (by default the name of source object)",
                   ""),
            o_comp("p", "comp", "<str>",
                   "transformation codes of the source complex object (by default 3,5)", "3,5"),
            o_filter("f", "filter", "<real>",
                     "phase filter limit in percent to absolute value (by default 1)", 1.0),
            o_channel("c", "chan", "<int>", "channel of input signal/spectrum (by default 0)", 0),
            o_voices("v", "voices", "<unsigned>",
                     "ASCII mode only: reduce spectrum's matrix to 'voices' rows (by default 256)",
                     256),
            o_points(
                    "s", "points", "<unsigned>",
                    "ASCII mode only: reduce spectrum's matrix to 'points' colums (by default 256)",
                    256),
            o_degree("d", "degree", "if set, the angular values will be written in degrees", false),
            o_modpi("1",
                    "modpi",
                    "if set, the modulo pi will be applied to angular values (only valid for: 3D azimuth)",
                    false)
    {
    ...
    void calc (void)
    {
        if (o_comp.isOptionGiven()) aComp.strToVector("{" + string(o_comp.getValue()) + "}");
        if (aComp.size() > 0)
        {
            aChann = o_channel.getValue();
            aFilter = o_filter.getValue();
            calc(ATypeMain::aSource);
            if (o_name.isOptionGiven()) ATypeMain::aDest.setObjectName(o_name.getValue());
    ...
    void calc (PPPSignalContainer<PPPcomplex> &aData)
    {
        ...
        for (unsigned i = 0; i < newsize; i++)
        {
            ...
            {
            case TCre:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPcomplexRe());
                break;
            case TCim:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
            ...
            case TCarg:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPcomplexArg(o_degree.getValue()));
                break;
            case TCargf:
                aData.getChannel(ind2).compTransform(
                        ATypeMain::aDest.getChannel(i),
                        PPPcomplexArgFilter(maxmod, aFilter, -M_PI, o_degree.getValue()));
                break;
            case TCabsm:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPcomplexAbsM(maxmod));
                break;
            case TCargm:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPcomplexArgM(maxmod));
                break;
            default:
                ATypeMain::onError(gwlConvert_ERR01);
            }
            if (!atype)
                aInfo << "Transform complex signal to: "
                << ATypeMain::aDest.getChannel(i).getObjectName() << endl;
            else if (atype && i == 0) aInfo << "Transform complex signal to: "
                                      << ATypeMain::aDest.getChannel(i).getObjectName() << "["
                                      << ATypeMain::aDest.channels() << "]" << endl;
        }
    }
    ...
    void calc (PPPSignalContainer<PPPellipse2D> &aData)
    {
        ...
        for (unsigned i = 0; i < newsize; i++)
        {
            ...
            case TEtilt:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPellipse2Dtilt(o_degree.getValue()));
                break;
            case TEphasediff:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPellipse2Dphasediff(o_degree.getValue()));
                break;
            case TEwx:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPellipse2Dwx());
            ...
            if (!atype)
                aInfo << "Transform 2D elliptic signal to: "
                << ATypeMain::aDest.getChannel(i).getObjectName() << endl;
            else if (atype && i == 0) aInfo << "Transform 2D elliptic signal to: "
                                      << ATypeMain::aDest.getChannel(i).getObjectName() << "["
                                      << ATypeMain::aDest.channels() << "]" << endl;
        }
    }
    ...
    void calc (PPPSignalContainer<PPPellipse3D> &aData)
    {
        ...
        for (unsigned i = 0; i < newsize; i++)
        {
            ...
            case TEplancosx:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPellipse3Dplancosx());
                break;
            case TEplancosy:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPellipse3Dplancosy());
                break;
            case TEplancosz:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPellipse3Dplancosz());
                break;
            case TEsignratio:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPellipse3Dsignratio());
                break;
            case TEdip:
                aData.getChannel(ind2).compTransform(ATypeMain::aDest.getChannel(i),
                                                     PPPellipse3Ddip(o_degree.getValue()));
                break;
            case TEazimuth:
                aData.getChannel(ind2).compTransform(
                        ATypeMain::aDest.getChannel(i),
                        PPPellipse3Dazimuth(o_degree.getValue(), o_modpi.getValue()));
                break;
            default:
                ATypeMain::onError(gwlConvert_ERR01);
            }
            if (!atype)
                aInfo << "Transform 3D elliptic signal to: "
                << ATypeMain::aDest.getChannel(i).getObjectName() << endl;
            else if (atype && i == 0) aInfo << "Transform 3D elliptic signal to: "
                                      << ATypeMain::aDest.getChannel(i).getObjectName() << "["
                                      << ATypeMain::aDest.channels() << "]" << endl;
        }
    }
    ...
    void calc (PPPSpectrContainer<PPPcomplex> &aData)
    {
        bool atype = (aComp.size() == 1);
        unsigned newsize = (atype) ? aData.channels() : aComp.size();
        ATypeMain::aDest.prepare(aData.voices(), aData.points(), newsize, aData.getTime(),
                                 aData.getFreq(), aData.getObjectName());
        double maxmod = 0.0;
        for (unsigned i = 0; i < newsize; i++)
        {
            unsigned ind1 = (atype) ? 0 : i;
            ...
            if (!atype)
                aInfo << "Transform complex spectrum to: "
                << ATypeMain::aDest.getChannel(i).getObjectName() << endl;
            else if (atype && i == 0) aInfo << "Transform complex spectrum to: "
                                      << ATypeMain::aDest.getChannel(i).getObjectName() << "["
                                      << ATypeMain::aDest.channels() << "]" << endl;
        }
    }
    ...
    void calc (PPPSpectrContainer<PPPellipse2D> &aData)
    {
        ...
        for (unsigned i = 0; i < newsize; i++)
        {
            ...
            if (!atype)
                aInfo << "Transform 2D elliptic spectrum to: "
                << ATypeMain::aDest.getChannel(i).getObjectName() << endl;
            else if (atype && i == 0) aInfo << "Transform 2D elliptic spectrum to: "
                                      << ATypeMain::aDest.getChannel(i).getObjectName() << "["
                                      << ATypeMain::aDest.channels() << "]" << endl;
        }
    }
    ...
    void calc (PPPSpectrContainer<PPPellipse3D> &aData)
    {
        ...
        for (unsigned i = 0; i < newsize; i++)
        {
            ...
            if (!atype)
                aInfo << "Transform 3D elliptic spectrum to: "
                << ATypeMain::aDest.getChannel(i).getObjectName() << endl;
            else if (atype && i == 0) aInfo << "Transform 3D elliptic spectrum to: "
                                      << ATypeMain::aDest.getChannel(i).getObjectName() << "["
                                      << ATypeMain::aDest.channels() << "]" << endl;
        }
    }
