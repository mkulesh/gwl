...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_name("n", "name", "<str>", "name of the output signal (by default 'signal')",
                   "signal"),
            o_format("f", "format", "<str>",
                     "format of input data (ASCII, STA. By default 'ASCII')", "ASCII"),
            o_type("y", "type", "<str>",
                   "structure of output signal (func, seis, seis2D. By default 'func')", "func"),
            o_istime("e", "istime", "if set, the time column is present in the input file", false),
            o_smplfreq("s", "smplfreq", "<real>",
                       "sampling frequency of source signal (by default 100)", 100.0),
            o_tmin("1", "tmin", "<real>", "minimum value of time of source signal (by default 0)",
                   0.0),
            o_tmax("2", "tmax", "<real>",
                   "maximum value of time frequency of source signal (by default 0)", 0.0),
            o_to2p("p", "to2p", "if set, the signal will be expand to power of two", false),
            o_chan("a",
                   "chan",
                   "<str>",
                   "channel(s) of source file which will be readed \"chan1,chan2,...chanN\" (by default '')",
                   ""),
            o_geo("g", "geo", "<unsigned>", "index of geophone for SAT format (by default 0)", 0),
            o_rot("r",
                  "rot",
                  "<str>",
                  "parameters for rotation procedure of two channels \"chan1,chan2,angle\" (by default '')",
                  ""),
            o_resample("l", "resample", "<unsigned>", "resampling of seismograms (by default '0')",
                       0),
            o_mult("u", "mult", "<real>",
                   "multiplication of all seismograms with a constant (by default '1')", 1)
    {
    ...
    void read (void)
    {
        ...
        inputTypes["STA"] = 2;
        inputTypes["WAV"] = 3;
        if (inputTypes.find(o_format.getValue()) == inputTypes.end()) PPPBaseObject::onError(
        gwlSignalRead_ERR01 + string("read()"));
        
        map<string, unsigned> outputTypes;
        outputTypes["func"] = 1;
        outputTypes["seis"] = 2;
        outputTypes["seis2D"] = 3;
        if (outputTypes.find(o_type.getValue()) == outputTypes.end()) PPPBaseObject::onError(
        gwlSignalRead_ERR01 + string("read()"));
        ...
        {
        case 1:
            if (aPar.getChannels().size() == 0) aPar.getChannels().strToVector("{0,1}");
            ...
            break;
        case 2:
            if (o_rot.isOptionGiven()) aPar.getRotation().strToVector(
                    "{" + string(o_rot.getValue()) + "}");
            if (inputTypes[o_format.getValue()] == 1)
        ...
        case 3:
            ...
            if (ATypeMain::aFileNames.size() != 2) PPPBaseObject::onError(
                    "too few or too many input signals are given");
