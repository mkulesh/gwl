...
public:
    ...
    void read (void)
    {
        ...
        for (unsigned k = 0; k < ATypeMain::aFileNames.size(); k++)
        {
            ...
            if (ATypeMain::aSource.channels() != ATypeMain::aDest.channels() || ATypeMain::aSource
                    .points()
                                                                                != ATypeMain::aDest
                                                                                        .points()) ATypeMain::onError(
                    "invalid size of the source signal " + ATypeMain::aFileNames[k]);
