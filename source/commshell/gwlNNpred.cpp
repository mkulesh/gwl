...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_name("n", "name", "<str>", "name of the predicted signal (by default 'prediction')",
                   "prediction"),
            ...
            o_nncount("u", "nncount", "<unsigned>", "count of nearest neighbors (by default 10)",
                      10),
            o_pcoeff("f", "pcoeff", "<real>",
                     "amplitude factor for calculation of farecasting (by default 1) ", 1.0),
            ...
            o_l3("3", "l3",
                 "if set, use L1-adaptive norm for nearest neighbors searching (by default 'true')",
                 true),
            o_trend("0", "trend", "<int>",
                    "power of polynomial o_trend; if = 0, then without trend (by default 0)", 0),
            o_sub("b", "sub", "<real>",
                  "substituation in the case of zero minimal element (by default 0) ", 0.0)
    {
    ...
    void calc (void)
    {
        ...
        if (o_trend.isOptionGiven() && o_trend.getValue() > 0) pred.extractTrend(aSource,
                                                                                 o_trend.getValue(),
                                                                                 true);
        
        // Nearest neighbors prediction
        pred.prepare(aSource, o_dim.getValue(), o_step.getValue());
        int norm = (o_l1.isOptionGiven()) ? 1 : (o_l2.isOptionGiven()) ? 2 :
                   (o_l3.isOptionGiven()) ? 3 : 0;
        pred.evalPrediction(aDest, o_plength.getValue(), o_nncount.getValue(), norm,
                            o_pcoeff.getValue(), o_sub.getValue());
        aDest.setObjectName(o_name.getValue());
        
        // Restoring of o_trend
        if (o_trend.isOptionGiven()) pred.restoreTend(aDest, aSource.size());
    }
