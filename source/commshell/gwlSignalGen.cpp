...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_type("y", "type", "<str>", "type of synthetic signal (by default 'harmon')",
                   "harmon"),
            o_par("p", "par", "<str>", "parameters for signal generation (by default '')", ""),
            o_modul("d", "modul", "<str>",
                    "parameters for windowed modulation of signal (by default '')", ""),
    ...
    void calc (void)
    {
        ...
        PPPSIGNALGEN_ERRTYPE + string("calc()"));
        
        PPPSignalGen<AType> aPar;
        aPar.getParams().strToVector("{" + string(o_par.getValue()) + "}");
        // Generation of signal
        ATypeMain::aDest.prepare(ATypeMain::aSource.size(), 1, ATypeMain::aSource,
                                 o_name.getValue());
        aPar.setShowNotation(PPPBaseObject::NMappend);
        switch (typesMap[o_type.getValue()])
        {
        case 0:
            aPar.genZero(ATypeMain::aDest);
            break;
        case 1:
            aPar.genDelta(ATypeMain::aDest);
            break;
        case 2:
            aPar.genHarmon(ATypeMain::aDest);
            break;
        case 3:
            aPar.genHarmonRot(ATypeMain::aDest);
            break;
        case 4:
            aPar.genHarmonPhase(ATypeMain::aDest);
            break;
        case 5:
            aPar.genRickerDissip(ATypeMain::aDest);
            break;
        };
        // random noise
