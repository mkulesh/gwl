...
template<class ATypeMain> class gwlMain : public ATypeMain
  ...
  public:
    ...
    gwlMain(const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut):
      ...
      o_name   ("n",  "name",    "<str>", "name of the inverse signal (by default 'f-k image')", "f-k image"),
      o_vel    ("v",  "vel",     "<str>", "name of the file with velocity axis (by default 'vel.dat')", "vel.dat"),
      o_inter  ("e",  "inter",   "<str>", "type of the velocity interpolation: index, spline (by default 'index')", "index"),
      o_corr   ("l",  "corr",    "<str>", "type of component for correlation: abs, arg, cphase (by default 'arg')", "arg"),
    ...
    void calc(void) {
      map < string, PPPTransFK::InterpType > InterpType;
        ...
        if(InterpType.find(o_inter.getValue()) == InterpType.end())
          ATypeMain::onError(PPPTRANSFK_ERRINT+string("calc()"));
      map < string, PPPTransFK::CorrType > CorrType;
        ...
        if(CorrType.find(o_corr.getValue()) == CorrType.end())
          ATypeMain::onError(PPPTRANSFK_ERRCOR+string("calc()"));
