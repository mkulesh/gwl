...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_name("n", "name", "<str>", "name of the spectrum (by default 'wavelet spectrum')",
                   "wavelet spectrum"),
            o_wttype(
                    "y",
                    "wttype",
                    "<unsigned>",
                    "type of the continuous wavelet transform: 0-time integral, 1-fft based, 2-multi-convolution method (by default 1)",
                    1),
            o_freq("f", "freq", "<file>",
                   "input file with frequency axis (by default 'frequency.dat')", "frequency.dat"),
            o_cutoff("u", "cutoff", "<real>",
                     "wavelet cutoff for slow wavelet transform (by default 0.01)", 0.01),
            o_trace("a", "trace", "<file>",
                    "output file with calculation time trace (by default '')", ""),
            o_points("s", "points", "<unsigned>",
                     "if given, reduce wavelet spectrum to 'points' length", 0)
    {
    ...
    void calc (void)
    {
        ...
        ATypeMain::aSpectrumPar.initialize((unsigned) o_wttype.getValue(), aFreq,
        ...
        if (o_trace.isOptionGiven()) trans.setTraceName(o_trace.getValue());
        ...
        if (o_points.isOptionGiven() && o_points.getValue() != 0)
        {
            ...
            aNewSpec.assign(ATypeMain::aDest, 0, ATypeMain::aDest.voices(), 1, 0,
                            o_points.getValue(), aNewOffSet);
