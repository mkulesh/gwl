...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_name("n", "name", "<str>", "name of the maximul lines (by default 'maximum lines')",
                   "maximum lines"),
            o_type("y", "type", "<str>",
                   "type of maximum line calculation: amaxt, amaxf (by default amaxt)", "amaxt")
    {
    ...
    void calc (void)
    {
        ...
        if (typesMap.find(o_type.getValue()) == typesMap.end()) PPPBaseObject::onError(
                "type of maximum line calculation is incorrect");
