...
class gwlMain : public gwlMainObject< PPPAxis, PPPDispersionModel >
  ...
  public:
    ...
    gwlMain(const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut):
      ...
      o_name   ("n",  "name",   "<str>", "name of the model (by default 'dispersion model')", "dispersion model"),
      o_wn     ("w",  "wn",     "<str>", "type of the wave number approximation: vel, gauss, polin, bspline, colecole, twogauss (by default 'gauss')", "gauss"),
      o_wnpar  ("1",  "wnpar",  "<str>", "parameters of the wave number approximation (by default '')", ""),
      o_atn    ("a",  "atn",    "<str>", "type of the attenuation approximation: gauss, polin, bspline (by default 'polin')", "polin"),
      o_atnpar ("2",  "atnpar", "<str>", "parameters of the attenuation approximation (by default '0')", "0"),
    ...
    PPPApproximate::ApprType parseApprType(const char *aApprType) {
      ...
      if(typesMap.find(aApprType) == typesMap.end())
        PPPBaseObject :: onError(PPPDISPERSIONMODEL_ERRTYPE+string("calc()"));
    ...
    void calc(void) {
      ...
      aDest.prepare(parseApprType(o_wn.getValue()), wnpar, parseApprType(o_atn.getValue()), atnpar);
