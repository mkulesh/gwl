...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_name("n", "name", "<str>",
                   "name of the elliptic parameters (by default 'filtered spectrum')",
                   "filtered spectrum"),
    ...
    void parsePolarizationFilters ()
    {
        ...
        while (1)
        {
            ...
            aFilters.push_back(
                    PPPEllipse3Dfilter(typesMap[strname], aPars[0], aPars[1], aPars[0], aPars[1]));
        }
    }
    ...
    void calc (PPPSpectrContainer<PPPcomplex> &aLocDest, PPPSpectrContainer<PPPcomplex> &aLocSource)
    {
        ...
        aLocDest.prepare(aLocSource.voices(), aLocSource.points(), 3 * aFilters.size(),
                         aLocSource.getTime(), aLocSource.getFreq(), "filtered spectrum");
        // calculation
