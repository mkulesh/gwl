...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_name("n", "name", "<str>",
                   "name of the inverse signal (by default 'optimized dispersion model')",
                   "optimized dispersion model"),
            o_spec("s", "spec", "<file>", "input spectrum's file name (by default 'cwt.dat')",
                   "cwt.dat"),
            o_ospec("z", "ospec", "<file>",
                    "optimized spectrum's file name (by default 'cwtopt.dat')", "cwtopt.dat"),
            ...
            o_isacorr("a", "acorr", "if set, the input signal/spectrum is an autocorrelation",
                      false),
            o_cmpl("p", "cmpl", "<unsigned>",
                   "type of optimized component: 3-abs, 4-arg (by default 3)", 3),
            o_eps("e", "eps", "<real>", "maximal precision (by default 1e-08)", 1E-08),
            o_chan("h", "chan", "<str>",
                   "channels of input spectrum which will be optimized (by default '0,1')", "0,1")
    {
    ...
    void calc (void)
    {
        ...
        if (o_chan.isOptionGiven())
        {
            ...
            opti.prepare(aSpectr, ATypeMain::aSource, o_dist.getValue(), aPropType,
                         &getSpectrParams(), o_cmpl.getValue(), chan[0], chan[1]);
            opti.setParams(aParDouble);
            opti.setShowProgress(ATypeMain::o_progr.getValue());
            opti.optimization();
            str2 << opti.getInfo();
            ATypeMain::aDest.assign(opti.getModel());
            if (o_ospec.isOptionGiven()) ATypeMain::write_bindata(opti.getResult(),
                                                                  o_ospec.getValue());
        }
        else
        {
            ...
            opti.prepare(aSpectr, ATypeMain::aSource, o_dist.getValue(), aPropType,
                         &getSpectrParams(), o_cmpl.getValue());
            opti.setParams(aParDouble);
            opti.setShowProgress(ATypeMain::o_progr.getValue());
            opti.optimization();
            str2 << opti.getInfo();
            ATypeMain::aDest.assign(opti.getModel());
            if (o_ospec.isOptionGiven()) ATypeMain::write_bindata(opti.getResultSpec(),
                                                                  o_ospec.getValue());
        }
