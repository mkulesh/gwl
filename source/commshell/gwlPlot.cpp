...
class gwlMain : public QMainWindow
  ...
  public:
    ...
    gwlMain(const char *aAppName, const char *aModName):
      ...
      o_chanelsignal("u","chanelsignal", "<unsigned>","number of chanel of signal (by default 0)", 0),
      o_spectrum ("t", "spectrum", "<file>", "input file with spectrum data (by default spectrum.dat)", "spectrum.dat"),
      o_chanelspectrum ( "v", "chanelspectrum", "<unsigned>","number of chanel of spetrum (by default 0)", 0)
      {
      o_parser.add(o_signal);
      o_parser.add(o_chanelsignal);
      o_parser.add(o_spectrum);
      o_parser.add(o_chanelspectrum);
      ConApplication.setAppName(string("\n")+string(aAppName)+string(", ")+string(PPPCONF_VERSION));
    ...
    void evaluate(void) {
      ...
      if (o_spectrum.isOptionGiven())
        _readSpectrum(_spec,o_spectrum.getValue(),true);
      // reading signal
      if (o_signal.isOptionGiven())
        _signal.read(o_signal.getValue());
  ...
  private:
    ...
    void _readSpectrum(AType &aData, const char *aName, bool aCheckType=false) {
      ...
      if(infile == NULL)
        aData.onError(FILE_ERROPEN+string(aName));
      aData.fread(infile);
      if(aCheckType && strcmp(aData.getObjectVer(),PPPSPECTRCONTAINER_OBJVER) == 0)
        _spectrumPar.fread(infile);
      fclose(infile);
      };
    ...
    void _addSpectrumToPlot(unsigned actualRow, PPPSpectrContainer<double> &aSpec, unsigned aChann) {
      ...
      finterp = (aSpec.getFreq().getType() == PPPAxis::ATlin)? UTPlotRasterData::LIN : UTPlotRasterData::LOG;
      ...
      UTPlotRasterData * tmp = new UTPlotRasterData(
         aSpec.getChannel(aChann).begin(),                           // the data
      ...
      if (finterp == UTPlotRasterData::LOG)
        _centralPlot -> plot(actualRow,0) -> setAxisScaleEngine(QwtPlot::yLeft, new QwtLog10ScaleEngine());
