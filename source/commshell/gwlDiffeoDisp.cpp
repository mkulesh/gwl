...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_name("n", "name", "<str>",
                   "name of the propagated function (by default 'propagated signal')",
                   "propagated signal"),
            o_model("l", "model", "<file>",
                    "input file with dispersion model (by default 'dispmodel.dat')",
                    "dispmodel.dat"),
            o_dist("d", "dist", "<real>", "propagation distance (by default 0)", 0.0),
            o_step("s", "step", "<unsigned>", "count of iterations (by default 1)", 1),
            o_isacorr("a", "acorr", "if set, the input signal/spectrum is an autocorrelation",
                      false),
            o_decrs("e", "decrs", "<real>",
                    "decrement coefficient for source signal (by default 1)", 1.0),
            o_prop("p",
                   "prop",
                   "<unsigned>",
                   "type of the wavelet propagator: 1-standart, 2-modulus-argument, 3-causal, 4,5-elliptic (by default 1)",
                   1)
    {
    ...
    void calc (PPPSignalContainer<AType> &aData)
    {
        ...
        if (o_decrs.isOptionGiven()) for (unsigned i = 0; i < aFour.points(); i++)
            aFour(i, 0) = aFour(i, 0) / o_decrs.getValue();
        double DeltaDis = aPropag.getDistance();
        PPPPropagatorDiss::PropType aPropType =
                (o_isacorr.isOptionGiven()) ? PPPPropagatorDiss::STfourcross :
                                              PPPPropagatorDiss::STfour;
        for (unsigned i = 1; i < aChannels; i++)
        {
            if (aPropag.getModel().isAnalytical() && aPropag.getModel().getWn().getType()
                    == PPPApproximate::APTtwogauss) aPropag.getModel().getWn()[6] = aPropag
                    .getDistance();
    ...
    void calc (PPPSpectrContainer<PPPcomplex> &aData)
    {
        ...
        ATypeMain::aDest.prepare(aData.voices(), aData.points(), aChannels, aData.getTime(),
                                 aData.getFreq(), aData.getObjectName());
        ATypeMain::aDest.getChannel(0).assign(aData.getChannel(0));
        if (o_decrs.isParsed()) for (unsigned i = 0; i < ATypeMain::aDest.voices(); i++)
            for (unsigned j = 0; j < ATypeMain::aDest.points(); j++)
                ATypeMain::aDest(i, j, 0) = ATypeMain::aDest(i, j, 0) / o_decrs.getValue();
        double DeltaDis = aPropag.getDistance();
        PPPPropagatorDiss::PropType aPropType =
                (o_isacorr.isOptionGiven()) ? PPPPropagatorDiss::STwavcross :
                                              PPPPropagatorDiss::STwav;
        for (unsigned i = 1; i < aChannels; i++)
        {
            aPropag.evalWaveletPropag(aPropType, aCWT, aData, ATypeMain::aSpectrumPar,
                                      o_prop.getValue());
