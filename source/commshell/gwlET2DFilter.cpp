...
public:
    ...
    gwlMain (const char *aAppName, const char *aModName, const char *aDefIn, const char *aDefOut) :
            ...
            o_name("n", "name", "<str>",
                   ...
                   "elliptic parameters"),
            o_type("y", "type", "<str>",
                   "type of the elliptic machine: acovar, complex (by default 'complex')",
                   "complex"),
            o_filter("f", "filter", "<str>", "polarization filter parameters (by default '')", ""),
            o_elli("e", "elli", "<file>",
                   "input file elliptical parameters (by default 'elli.dat')", "elli.dat"),
            o_freq("q", "freq", "<file>",
                   "input file with frequency axis (by default 'frequency.dat')", "frequency.dat")
    {
    ...
    void parsePolarizationFilters ()
    {
        ...
        while (1)
        {
            ...
            if (typesMap.find(strname) == typesMap.end()) PPPBaseObject::onError(
            gwlET2D_ERR03 + strname);
            aStr.erase(0, pos + 1);
            string strpar = "";
            ...
            if (aPars.size() != 2) ATypeMain::onError(gwlET2D_ERR04);
    ...
    void calc (void)
    {
        ...
        if (o_filter.isOptionGiven()) parsePolarizationFilters();
        ...
        if (aFilters.size() > 0) calc(ATypeMain::aDest, ATypeMain::aSource);
    }
    
    void calc (PPPSpectrContainer<PPPcomplex> &aLocDest, PPPSpectrContainer<PPPcomplex> &aLocSource)
    {
        unsigned newchann =
                (aTrans.getMachine() == WTEComplex) ? aFilters.size() : 2 * aFilters.size();
        aLocDest.prepare(aLocSource.voices(), aLocSource.points(), newchann, aLocSource.getTime(),
                         aLocSource.getFreq(), "filtered spectrum");
        // calculation
    ...
    void calc (PPPSignalContainer<PPPcomplex> &aLocDest, PPPSignalContainer<PPPcomplex> &aLocSource)
    {
        ...
        ATypeMain::aSpectrumPar.initialize(1, aFreq, ATypeMain::o_wavelet.getValue(),
                                           ATypeMain::o_wavpar.getValue(), "delta", 1.0);
    ...
    void calcFullFilterComplex (vector<PPPSignalContainer<PPPcomplex> > &aRecSeis,
                                PPPSignalContainer<PPPcomplex> &aLocSource)
    {
        ...
        aCWTfil.prepare(aCWT.voices(), aCWT.points(), aFilters.size(), aCWT.getTime(),
                        aCWT.getFreq(), "filtered spectrum");
        char numchan[100];
    ...
    void calcFullFilterACovar (vector<PPPSignalContainer<PPPcomplex> > &aRecSeis,
                               PPPSignalContainer<PPPcomplex> &aLocSource)
    {
        ...
        for (unsigned j = 0; j < aLocSource.channels(); j++)
        {
            ...
            if (j == 0) aCWTfil.prepare(aCWT.voices(), aCWT.points(), 2 * aFilters.size(),
                                        aCWT.getTime(), aCWT.getFreq(), "filtered spectrum");
